...
const user = { id: 94, name: 'Marina Atendente', email: 'marina@test.com', role: 'FUNCIONARIO', subRole: 'ATENDENTE', restaurantId };
...
async function setup(page: Page) {
  const state = {
    ...
    orders: [
      { id: 'public-102', orderId: 102, code: '#102', type: 'DELIVERY', status: 'PENDENTE', tableNumber: null, customerName: 'Rui', createdAt: minutesAgo(38), readyAt: null, items: [{ quantity: 1, productName: 'Calzone' }] },
      { id: 'public-103', orderId: 103, code: '#103', type: 'RETIRADA', status: 'PRONTO', tableNumber: null, customerName: 'Bianca', createdAt: minutesAgo(11), readyAt: minutesAgo(2), items: [{ quantity: 1, productName: 'Combo família' }] },
    ],
    calls: [
      { id: '71', tableNumber: 8, type: 'BILL', status: 'WAITING', assignedToId: null as number | null, assignedToName: null as string | null, requestedAt: minutesAgo(9), assignedAt: null as string | null, resolvedAt: null as string | null },
    ],
    tables: [{ id: '8', tableNumber: 8, status: 'CLOSING_REQUESTED', openedAt: minutesAgo(90), participantCount: 4, activeOrderCount: 1, activeCallCount: 1 }],
  };
  ...
  await page.route(/^http:\/\/(127\.0\.0\.1|localhost):3000\/.*$/, async (route) => {
    ...
    if (path === `/settings/public/${restaurantId}`) return json(route, { primaryColor: '#e16a3d', restaurant: { id: restaurantId, name: 'Pizzaria Horizonte', slug: 'pizzaria-horizonte' } });
    ...
    if (path === '/orders/103') return json(route, { id: 103, type: 'RETIRADA', status: 'PRONTO', paid: true, total: 49.9, user: { name: 'Bianca', phone: '+5585999999999' }, items: [{ quantity: 1, product: { name: 'Combo família' } }] });
    if (path === '/orders/103/status' && request.method() === 'PUT') { state.orders = state.orders.filter((order) => order.orderId !== 103); return json(route, { id: 103, status: 'ENTREGUE' }); }
    if (path === '/attendant/calls/71/status') {
      const status = String((request.postDataJSON() as { status?: string }).status || '');
      const call = state.calls[0];
      if (status === 'IN_PROGRESS') { call.status = 'IN_PROGRESS'; call.assignedToId = user.id; call.assignedToName = user.name; call.assignedAt = new Date().toISOString(); }
      if (status === 'RESOLVED') { call.status = 'RESOLVED'; call.resolvedAt = new Date().toISOString(); }
      return json(route, call);
    }
    if (path === '/attendant/orders' && request.method() === 'POST') { manualPayload = request.postDataJSON() as Record<string, unknown>; return json(route, { id: 150, status: 'PENDENTE' }, 201); }
