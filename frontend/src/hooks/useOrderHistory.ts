...
export function useOrderHistory({
  ...
  const queryKey = JSON.stringify({ limit: HISTORY_BATCH_SIZE, queue: 'HISTORY', ...query });
  const read = useCallback(
    (cursor?: number) => {
      const params = { ...JSON.parse(queryKey), ...(cursor ? { cursor } : {}) };
      return mine
        ? ordersService.listMyOrders(params)
        : ordersService.listRestaurantOrdersPage(params);
    },
    [mine, queryKey],
  );
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      setPage((current) => ({
        ...
        orders: [
          ...new Map(
            [...current.orders, ...result.orders].map((order) => [
      ...
      if (version === request.current)
        setError('Não foi possível carregar mais pedidos. Tente novamente.');
    } finally {
      if (version === request.current) {
        busy.current = false;
        setLoading(false);
  ...
  const reset = async () => {
    ...
    try {
      ...
      if (version === request.current)
        setError('Não foi possível voltar aos 10 primeiros itens. Tente novamente.');
    } finally {
      if (version === request.current) {
        busy.current = false;
        setLoading(false);
  ...
  return {
    ...
    refresh,
    reset,
    loadedCount: page.orders.length,
