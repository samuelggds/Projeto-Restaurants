...
const allowed = (path: string, user: Parameters<typeof authorizeRoute>[1]) => authorizeRoute(path, user).allowed;

describe("política de autorização de rotas", () => {
  it("mantém Home e QR públicos, mas protege tracking", () => {
    for (const path of ["/", "/mesa/12", "/pizzaria", "/pizzaria/mesa/12"]) expect(allowed(path, null), path).toBe(true);
    expect(authorizeRoute("/orders/42/tracking", null)).toEqual({ allowed: false, redirectTo: "/login" });
  });
  it("limita CLIENTE a Home, cardápio, perfil e tracking", () => {
    const user = { role: "CLIENTE" };
    for (const path of ["/", "/loja/mesa/3", "/profile", "/orders/a/tracking"]) expect(allowed(path, user), path).toBe(true);
  ...
  it("permite todos os módulos operacionais ao ADMIN, menos super_admin", () => {
    ...
    for (const path of ["/", "/admin", "/billing", "/profile", "/orders/1/tracking", "/courier", "/kitchen", "/waiter"]) expect(allowed(path, user), path).toBe(true);
  ...
  it("isola motoqueiro, cozinha e garçom", () => {
    const cases = [[{ role: "MOTOQUEIRO" }, "/courier", "/admin"], [{ role: "FUNCIONARIO", subRole: "COZINHA" }, "/kitchen", "/waiter"], [{ role: "FUNCIONARIO", subRole: "GARCOM" }, "/waiter", "/kitchen"]] as const;
  ...
  it("manda perfil desconhecido para login", () => {
    expect(authorizeRoute("/admin", { role: "OUTRO" })).toEqual({ allowed: false, redirectTo: "/login" });
  });
});
