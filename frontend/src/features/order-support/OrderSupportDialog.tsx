...
export function OrderSupportDialog({
  ...
  const initial = initialOrderId || (orders.length === 1 ? orders[0]?.id : null);
  const [selectedOrderId, setSelectedOrderId] = useState<number | null>(initial || null);
  ...
  useEffect(() => {
    if (!open) return;
    setSelectedOrderId(initial || null);
    setCategory('');
    setThread(null);
    setDraft('');
    setError('');
  ...
  const canCompose = Boolean(selectedOrderId && !thread?.isResolved && (hasConversation || category));
  ...
  return (
    <Backdrop role="presentation" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <Dialog role="dialog" aria-modal="true" aria-label="Central de suporte do pedido">
        <Header>
          <div>
            ...
            <span>
              ...
              <small>{visitor ? 'Suporte seguro para seu pedido' : 'Fale com o restaurante sobre um pedido'}</small>
            </span>
          </div>
          <button type="button" onClick={onClose} aria-label="Fechar central de ajuda"><X /></button>
        </Header>
        ...
        <Body>
          <OrderRail $hidden={Boolean(selectedOrderId)}>
            ...
            <OrderList>
              {orders.map((order) => (
                <OrderButton key={order.id} type="button" onClick={() => setSelectedOrderId(order.id)}>
                  <span>
                    <b>Pedido #{order.id}</b>
          ...
          <Conversation $visible={Boolean(selectedOrderId)}>
            {selectedOrder ? (
              <ConversationHeader>
                ...
                <span>
                  ...
                  <small>{displayStatus(selectedOrder.status)} {money(selectedOrder.total) ? `• ${money(selectedOrder.total)}` : ''}</small>
                </span>
              </ConversationHeader>
            ) : null}
            ...
            <ConversationMain>
              {loading ? (
                ...
                <State $error><AlertCircle /><strong>{error}</strong><span>Confira se este pedido pertence a você e tente novamente.</span></State>
              ) : !hasConversation ? (
                <StartSupport>
                  <div className="intro">
                  ...
                  {category ? <Hint>Conte em poucas palavras o que aconteceu. O número do pedido será enviado automaticamente.</Hint> : null}
                </StartSupport>
              ) : (
                  ...
                  {thread?.messages.map((message, index) => {
                    ...
                    return (
                      <Bubble key={String(message.id || `${message.sentAt}-${index}`)} $client={client}>
                        ...
                        <p>{message.message}</p>
                        {message.sentAt ? <time>{new Date(message.sentAt).toLocaleString('pt-BR', { hour: '2-digit', minute: '2-digit', day: '2-digit', month: '2-digit' })}</time> : null}
                      </Bubble>
                    );
                  })}
                  <div ref={endRef} />
            ...
            {thread?.isResolved ? (
              <Resolved><CheckCircle2 /><span><b>Atendimento resolvido</b><small>Esta conversa está disponível para consulta.</small></span></Resolved>
            ) : selectedOrderId ? (
              <ComposerArea>
                ...
                <Composer onSubmit={submit}>
                  <textarea
                    ...
                    rows={2}
                  />
                  <button type="submit" disabled={!canCompose || !draft.trim() || sending} aria-label="Enviar mensagem">
                    <Send />
                  </button>
                </Composer>
                <small>{visitor ? 'Seu acesso é protegido pelo comprovante seguro deste pedido.' : 'A equipe verá automaticamente o número e os dados deste pedido.'}</small>
              </ComposerArea>
            ) : null}
          </Conversation>
        </Body>
...
`;
const Body = styled.div`min-height:0;display:grid;grid-template-columns:minmax(280px,330px) minmax(0,1fr);@media(max-width:720px){grid-template-columns:1fr;}`;
const OrderRail = styled.aside<{ $hidden:boolean }>`min-height:0;overflow:auto;padding:16px;background:#f8fafc;border-right:1px solid #e7edf5;@media(max-width:720px){display:${p=>p.$hidden?'none':'block'};border-right:0;}`;
const RailIntro = styled.div`padding:8px 4px 16px;display:grid;gap:5px;svg{width:25px;color:#d65632;}strong{font-size:14px;}span{font-size:11px;color:#64748b;line-height:1.45;}`;
const OrderList = styled.div`display:grid;gap:8px;`;
const OrderButton = styled.button`width:100%;padding:12px;display:flex;justify-content:space-between;gap:10px;text-align:left;border:1px solid #e2e8f0;border-radius:13px;background:#fff;cursor:pointer;transition:.16s;span{min-width:0;}b,small{display:block;}b{font-size:12px;}small{margin-top:3px;color:#64748b;font-size:10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}.meta{text-align:right;flex:0 0 auto;}.meta em{display:block;color:#27804b;font-size:9px;font-style:normal;font-weight:800;margin-bottom:4px;}.meta b{font-size:11px;} &:hover{border-color:#edb9a7;box-shadow:0 8px 20px rgba(15,23,42,.06);}`;
const Conversation = styled.section<{ $visible:boolean }>`min-height:0;min-width:0;display:grid;grid-template-rows:auto minmax(0,1fr) auto;background:#fff;@media(max-width:720px){display:${p=>p.$visible?'grid':'none'};}`;
const ConversationHeader = styled.header`min-height:62px;padding:10px 14px;display:flex;align-items:center;gap:10px;border-bottom:1px solid #e7edf5;button{width:38px;height:38px;display:grid;place-items:center;border:1px solid #dce4ef;border-radius:10px;background:#fff;cursor:pointer;}button svg{width:18px;}span{min-width:0;}strong,small{display:block;}strong{font-size:13px;}small{margin-top:2px;color:#64748b;font-size:10px;}`;
const ConversationMain = styled.div`min-height:0;min-width:0;overflow:auto;background:#f8fafc;`;
const State = styled.div<{ $error?:boolean }>`height:100%;min-height:240px;padding:28px;display:grid;place-items:center;align-content:center;gap:7px;text-align:center;color:${p=>p.$error?'#b42318':'#64748b'};svg{width:28px;}strong{font-size:13px;}span{max-width:330px;font-size:11px;line-height:1.45;}`;
const StartSupport = styled.div`padding:22px;display:grid;gap:18px;.intro span{font-size:10px;color:#d65632;font-weight:900;text-transform:uppercase;letter-spacing:.06em;}.intro strong{display:block;margin-top:4px;font-size:19px;}.intro p{margin:5px 0 0;color:#64748b;font-size:11px;line-height:1.5;}@media(max-width:480px){padding:16px;}.intro strong{font-size:17px;}`;
const CategoryGrid = styled.div`display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px;button{min-height:62px;padding:10px;display:flex;align-items:center;gap:9px;border:1px solid #dde5ef;border-radius:13px;background:#fff;color:#334155;text-align:left;font-size:11px;font-weight:800;cursor:pointer;}button svg{width:19px;flex:0 0 auto;color:#d65632;}button.active{border-color:#df7656;background:#fff5f1;box-shadow:0 0 0 2px rgba(214,86,50,.08);}@media(max-width:380px){grid-template-columns:1fr;button{min-height:52px;}}`;
...
const Bubble = styled.div<{ $client:boolean }>`align-self:${p=>p.$client?'flex-end':'flex-start'};max-width:min(78%,480px);padding:9px 11px;border:1px solid ${p=>p.$client?'#f1c7b8':'#e2e8f0'};border-radius:${p=>p.$client?'14px 14px 4px 14px':'14px 14px 14px 4px'};background:${p=>p.$client?'#fff5f1':'#fff'};b{display:block;margin-bottom:3px;font-size:9px;color:#64748b;}p{margin:0;font-size:12px;line-height:1.45;white-space:pre-wrap;}time{display:block;margin-top:4px;text-align:right;color:#94a3b8;font-size:8px;}@media(max-width:480px){max-width:88%;}`;
...
const Composer = styled.form`display:grid;grid-template-columns:minmax(0,1fr) 44px;gap:8px;textarea{width:100%;min-width:0;max-height:100px;resize:none;padding:10px 11px;border:1px solid #cfd8e5;border-radius:12px;font:inherit;font-size:13px;outline:0;}textarea:focus{border-color:#d65632;box-shadow:0 0 0 3px rgba(214,86,50,.10);}button{border:0;border-radius:12px;background:#d65632;color:#fff;display:grid;place-items:center;cursor:pointer;}button:disabled{opacity:.45;cursor:not-allowed;}button svg{width:18px;}@media(max-width:720px){textarea{font-size:16px;}}`;
