...
export function OrderSupportDialog({
  ...
  const dialogRef = useDialogFocusManagement<HTMLElement>(onClose, open);
  ...
  return createPortal(
    ...
    >
      <Dialog
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Central de suporte do pedido"
      >
        <Header>
          <div>
            <span className="icon">
            ...
            <span>
              ...
              <small>
                {visitor
                  ? 'Suporte seguro para seu pedido'
                  : 'Fale com o restaurante sobre um pedido'}
              </small>
            </span>
          </div>
          <button type="button" onClick={closeAndReset} aria-label="Fechar central de ajuda">
            <X />
          </button>
        </Header>
          ...
          <Conversation $visible={Boolean(selectedOrderId)}>
            ...
            <ConversationMain>
              {loading ? (
                <State>
                  <MessageCircle />
                  <strong>Carregando atendimento...</strong>
                </State>
              ) : error && !thread ? (
                <State $error>
                  <AlertCircle />
                ...
                <StartSupport>
                  <div className="intro">
                    ...
                    <p>
                      Isso ajuda o restaurante a entender sua necessidade antes mesmo de responder.
                    </p>
                  </div>
                  <CategoryGrid>
                  ...
                  {category ? (
                    <Hint>
                      Conte em poucas palavras o que aconteceu. O número do pedido será enviado
                      automaticamente.
                    </Hint>
                  ) : null}
                </StartSupport>
                ...
                <Messages aria-live="polite">
                  <SystemNote>
                    <CheckCircle2 /> Atendimento vinculado ao Pedido #{selectedOrderId}. Não é
                    necessário informar seus dados novamente.
                  </SystemNote>
                  {thread?.messages.map((message, index) => {
                    const client = String(message.senderType || '').toUpperCase() === 'CLIENT';
                    return (
                      <Bubble
                        key={String(message.id || `${message.sentAt}-${index}`)}
                        $client={client}
                      >
            ...
            {thread?.isResolved ? (
              <Resolved>
                <CheckCircle2 />
                <span>
                  <b>Atendimento resolvido</b>
                  <small>Esta conversa está disponível para consulta.</small>
                </span>
              </Resolved>
            ) : selectedOrderId ? (
              <ComposerArea>
...
const State = styled.div<{ $error?: boolean }>`
  ...
  gap: 7px;
  text-align: center;
  color: ${(props) => (props.$error ? '#b42318' : '#64748b')};
  svg {
    width: 28px;
  }
  strong {
...
const CategoryGrid = styled.div`
  ...
  button.active {
    border-color: #df7656;
    background: #fff5f1;
    box-shadow: 0 0 0 2px rgba(214, 86, 50, 0.08);
  }
...
const Bubble = styled.div<{ $client: boolean }>`
  align-self: ${(props) => (props.$client ? 'flex-end' : 'flex-start')};
  max-width: min(78%, 480px);
  padding: 9px 11px;
  border: 1px solid ${(props) => (props.$client ? '#f1c7b8' : '#e2e8f0')};
  border-radius: ${(props) => (props.$client ? '14px 14px 4px 14px' : '14px 14px 14px 4px')};
  background: ${(props) => (props.$client ? '#fff5f1' : '#fff')};
  b {
    display: block;
    margin-bottom: 3px;
    font-size: 9px;
...
`;
const ComposerArea = styled.footer`
...
const Composer = styled.form`
  ...
  textarea:focus {
    border-color: #d65632;
    box-shadow: 0 0 0 3px rgba(214, 86, 50, 0.1);
  }
  button {
    border: 0;
    border-radius: 12px;
