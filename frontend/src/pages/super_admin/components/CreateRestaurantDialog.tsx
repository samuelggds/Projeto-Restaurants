...
export function CreateRestaurantDialog({ onClose, onCreated }: Props) {
  ...
  const setRestaurant = (
    field: keyof CreateRestaurantPayload["restaurant"],
    value: string,
  ) => {
    setForm((current) => ({
      ...current,
      restaurant: {
        ...current.restaurant,
        [field]: value,
        ...(field === "name" && !current.restaurant.slug
          ? { slug: slugify(value) }
          : {}),
      },
    }));
  ...
  const setAdmin = (
    field: keyof CreateRestaurantPayload["admin"],
    value: string,
  ) => {
    setForm((current) => ({
      ...current,
    ...
    <S.CreateBackdrop role="presentation" onMouseDown={onClose}>
      ...
      >
        ...
        <div className="fields">
          <label>
            Nome do restaurante
            <input required minLength={2} value={form.restaurant.name} onChange={(event) => setRestaurant("name", event.target.value)} />
          </label>
          <label>
            Endereço da loja (slug)
            <input required minLength={3} value={form.restaurant.slug} onChange={(event) => setRestaurant("slug", slugify(event.target.value))} />
          </label>
          <label>
            E-mail do restaurante
            <input required type="email" value={form.restaurant.email} onChange={(event) => setRestaurant("email", event.target.value)} />
          </label>
          <label>
            Telefone
            <input inputMode="numeric" value={form.restaurant.phone} onChange={(event) => setRestaurant("phone", event.target.value)} />
          </label>
          <label>
            Nome do administrador
            <input required minLength={2} value={form.admin.name} onChange={(event) => setAdmin("name", event.target.value)} />
          </label>
          <label>
            E-mail do administrador
            <input required type="email" value={form.admin.email} onChange={(event) => setAdmin("email", event.target.value)} />
          </label>
          <label>
            Senha inicial
            <input required type="password" minLength={6} value={form.admin.password} onChange={(event) => setAdmin("password", event.target.value)} />
          </label>
          <label>
            Plano escolhido pelo cliente
            <select value={form.plan} onChange={(event) => setForm((current) => ({ ...current, plan: event.target.value as CreateRestaurantPayload["plan"] }))}>
