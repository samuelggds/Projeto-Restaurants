...
export function mapRestaurantTenant(
  record: Record<string, unknown>,
): RestaurantTenant {
  const owner = (record.owner as Record<string, unknown> | null) ?? null;
  const subscription =
    (record.subscription as Record<string, unknown> | null) ?? null;
  return {
    id: String(record.id),
    ...
    status: mapTenantStatus(record.status),
    createdAt: record.createdAt
      ? new Date(String(record.createdAt)).toLocaleDateString("pt-BR")
      : "—",
    lastAccess: "—",
    nextBillingAt: record.nextBillingAt
      ? new Date(String(record.nextBillingAt)).toLocaleDateString("pt-BR")
      : null,
...
export function derivePlans(restaurants: RestaurantTenant[]): Plan[] {
  const counts = restaurants.reduce<Record<string, number>>(
    (result, restaurant) => {
      const key = restaurant.plan.toUpperCase();
  ...
  return Object.entries(PLAN_META).map(([id, meta]) => ({
    id,
    ...meta,
    restaurants: counts[id] || 0,
  }));
}

export function deriveAdministrators(
  restaurants: RestaurantTenant[],
): PlatformAdministrator[] {
  return restaurants
    .filter((restaurant) => restaurant.responsible !== "—")
    .map((restaurant) => ({
      id: restaurant.id,
      name: restaurant.responsible,
      ...
      status:
        restaurant.status === "BLOCKED"
          ? ("BLOCKED" as const)
          : ("ACTIVE" as const),
      lastAccess: restaurant.lastAccess,
      twoFactor: false,
    }));
}
...
export function buildPlatformMetrics(
  ...
  return {
    ...
    restaurantsTrial: restaurants.filter((item) => item.status === "TRIAL")
      .length,
    restaurantsOverdue: restaurants.filter((item) => item.status === "OVERDUE")
      .length,
    restaurantsBlocked: restaurants.filter((item) => item.status === "BLOCKED")
      .length,
    restaurantsCanceled: restaurants.filter((item) => item.status === "CANCELED")
      .length,
    ...
    monthlyGrowth:
      (backend.monthlyGrowth as { label: string; count: number }[]) || [],
    monthlyRevenue:
      (backend.monthlyRevenue as { label: string; value: number }[]) || [],
  };
}
