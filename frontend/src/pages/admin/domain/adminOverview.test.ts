...
import { calculateOverviewMetrics, filterCustomerSummaries, summarizeCustomers } from "./adminOverview";
...
describe("visão geral administrativa", () => {
  ...
  it("filtra clientes por nome ou e-mail", () => {
    const customers = summarizeCustomers([
      ...
      order({ numericId: 2, id: "#2", customerName: "Maria Souza", customerEmail: "maria@teste.com" }),
    ]);
    expect(filterCustomerSummaries(customers, "MARIA")).toEqual([expect.objectContaining({ name: "Maria Souza" })]);
    expect(filterCustomerSummaries(customers, "joao@teste.com")).toEqual([expect.objectContaining({ name: "João Silva" })]);
  });
});
