  ...
  return fallback.map((day) => {
    ...
    return saved
      ? {
          ...
          openingTime: /^\d{2}:\d{2}$/.test(String(saved.openingTime))
            ? String(saved.openingTime)
            : day.openingTime,
          closingTime: /^\d{2}:\d{2}$/.test(String(saved.closingTime))
            ? String(saved.closingTime)
            : day.closingTime,
        }
      : day;
  });
...
export function validateBusinessHours(hours: BusinessHour[]): BusinessHoursErrors {
  ...
  hours.forEach((day) => {
    if (
      day.enabled &&
      (!/^\d{2}:\d{2}$/.test(day.openingTime) ||
        !/^\d{2}:\d{2}$/.test(day.closingTime) ||
        day.openingTime >= day.closingTime)
    )
      errors[day.id] = 'O fechamento deve ser depois da abertura.';
  });
  return errors;
}
