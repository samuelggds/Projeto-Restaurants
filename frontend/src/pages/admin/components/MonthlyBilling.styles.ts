...
export const Summary = styled.div`
  ...
  border-radius: 18px;
  background: linear-gradient(135deg, #191c1f, #29241f);
  color: #fff;
  padding: 24px;
  display: grid;
  ...
  gap: 20px;
  align-items: center;
  box-shadow: 0 16px 36px rgba(38, 28, 20, 0.1);
  h2 {
    margin: 7px 0;
    font-size: 25px;
  }
...
export const Status = styled.span<{ $active: boolean }>`
  ...
  align-items: center;
  gap: 8px;
  background: ${({ $active }) => ($active ? '#eaf7eb' : '#fff0ed')};
  color: ${({ $active }) => ($active ? '#26753a' : '#a53f35')};
  font-size: 12px;
  font-weight: 800;
  white-space: nowrap;
  &::before {
...
export const PlanCard = styled.article<{ $featured?: boolean; $current?: boolean }>`
  ...
  min-height: 365px;
  overflow: hidden;
  border: 2px solid ${({ $current }) => ($current ? 'var(--a)' : 'var(--border)')};
  border-radius: 20px;
  background: ${({ $featured }) => ($featured ? 'linear-gradient(145deg,#fff7ef 0%,#fff 58%)' : '#fff')};
  padding: 24px;
  display: flex;
  flex-direction: column;
  gap: 15px;
  box-shadow: ${({ $current, $featured }) => ($current ? '0 18px 42px rgba(116,66,31,.15)' : $featured ? '0 14px 34px rgba(77,47,25,.09)' : '0 8px 24px rgba(43,31,22,.06)')};
  transition:
  ...
  &::before {
    ...
    inset: 0 auto 0 0;
    width: 4px;
    background: ${({ $current, $featured }) => ($current || $featured ? 'var(--a)' : 'transparent')};
  }
  &:hover {
    transform: translateY(-3px);
    box-shadow: 0 20px 42px rgba(43, 31, 22, 0.13);
    border-color: color-mix(in srgb, var(--a) 52%, var(--border));
  }
  .description {
    margin: 0;
    min-height: 42px;
...
export const ChoosePlanButton = styled.button<{ $current: boolean }>`
  ...
  margin-top: auto;
  border: 1px solid
    ${({ $current }) => ($current ? 'color-mix(in srgb,var(--a) 32%,#ddd)' : 'var(--a)')};
  border-radius: 12px;
  background: ${({ $current }) => ($current ? 'color-mix(in srgb,var(--a) 8%,#fff)' : 'var(--a)')};
  ...
  &:hover:not(:disabled) {
    ...
    box-shadow: 0 9px 20px color-mix(in srgb, var(--a) 25%, transparent);
  }
  &:disabled {
    cursor: not-allowed;
    opacity: 0.82;
...
export const CurrentTag = styled.span`
  ...
  font-weight: 850;
`;
export const RecommendedTag = styled(CurrentTag)`
  background: #211d1a;
  color: #fff;
`;
...
export const ViewTabs = styled.div`
  ...
  button.active {
    background: #fff;
    color: var(--a);
    box-shadow: 0 5px 14px rgba(43, 31, 22, 0.09);
  }
...
export const PixModal = styled.div`
  ...
  background: #fff;
  padding: 24px;
  box-shadow: 0 28px 80px rgba(20, 14, 10, 0.3);
  position: relative;
  text-align: center;
  .close {
    position: absolute;
