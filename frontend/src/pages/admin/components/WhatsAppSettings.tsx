...
export function WhatsAppSettings({ settings, update }: Props) {
  ...
  const chooseImage = (file?: File) => {
    if (!file) return;
    setImageError('');
    if (!/^image\/(png|jpeg|webp)$/u.test(file.type)) return setImageError('Escolha uma imagem PNG, JPG ou WEBP.');
    if (file.size > MAX_PROFILE_IMAGE_BYTES) return setImageError('A imagem deve ter no máximo 500 KB.');
  ...
  const removeImage = () => { window.localStorage.removeItem(storageKey); setProfileImage(''); setImageError(''); };

  return (
    <Panel>
      <header className="wa-header">
        <div className="wa-title"><span aria-hidden="true"><MessageCircle size={24} /></span><div><h2>Configurar WhatsApp</h2><p>Configure o contato e veja como o restaurante aparecerá no atendimento.</p></div></div>
        <label className="channel-state"><span><b>{enabled ? 'Canal ativo' : 'Canal desativado'}</b><small>{enabled && number ? `+${number}` : 'Ative quando estiver pronto'}</small></span><input className="switch" name="whatsappEnabled" type="checkbox" role="switch" aria-label="Ativar WhatsApp do restaurante" checked={enabled} onChange={(event) => update('whatsappEnabled', event.target.checked)} /></label>
      </header>

      <div className="wa-grid">
        <div className="left-column">
          <section className="card">
            <header className="card-heading"><span className="step">1</span><div><h3>Foto e nome do perfil</h3><p>Escolha como o restaurante será apresentado nas mensagens.</p></div></header>
            <div className="profile-layout">
              <div className="profile-preview"><img className={`profile-image ${profileImage ? 'custom' : ''}`} src={avatar} alt="Foto do perfil do WhatsApp" /><b>{displayName}</b></div>
              <div><div className="profile-actions"><button className="profile-action" type="button" onClick={() => inputRef.current?.click()}><ImagePlus size={14} /> {profileImage ? 'Alterar foto' : 'Escolher foto'}</button>{profileImage ? <button className="profile-action danger" type="button" onClick={removeImage}><Trash2 size={14} /> Remover</button> : null}<input ref={inputRef} type="file" accept="image/png,image/jpeg,image/webp" hidden onChange={(event) => chooseImage(event.target.files?.[0])} /></div><span className="help">Por padrão usamos a logo preto e branco da categoria, sem fundo. A foto escolhida fica exclusiva desta configuração de WhatsApp.</span>{imageError ? <span className="error">{imageError}</span> : null}<S.Field style={{ marginTop: 13 }}>Nome no WhatsApp<input name="whatsappDisplayName" maxLength={80} placeholder="Ex.: North Pizza" value={settings.whatsappDisplayName} onChange={(event) => update('whatsappDisplayName', event.target.value)} /><small className="help">Se ficar vazio, usamos o nome do restaurante.</small></S.Field></div>
            </div>
          </section>

          <section className="card"><header className="card-heading"><span className="step">2</span><div><h3>Seu número do WhatsApp</h3><p>Informe o número comercial usado pelo restaurante no atendimento.</p></div></header><S.Field>Número comercial<input name="whatsapp" inputMode="tel" autoComplete="tel" placeholder="Ex.: 55 11 99999-9999" value={settings.whatsapp} aria-invalid={Boolean(numberError)} onChange={(event) => update('whatsapp', event.target.value)} /><small className={numberError ? 'error' : 'help'}>{numberError || 'Use DDI + DDD + número. Ex.: 55 11 99999-9999.'}</small></S.Field></section>
...
function Automation({ icon, title, text }: { icon: React.ReactNode; title: string; text: string }) { return <div className="automation-row"><span className="automation-icon">{icon}</span><span className="automation-copy"><b>{title}</b><span>{text}</span></span><span className="badge">AUTOMÁTICO</span></div>; }
function PreviewMessage({ avatar, custom, name, children }: { avatar: string; custom: boolean; name: string; children: React.ReactNode }) { return <div className="chat-message"><img className={`chat-avatar ${custom ? 'custom' : ''}`} src={avatar} alt="" aria-hidden="true" /><div className="message-content"><b>{name}</b><div className="bubble">{children}</div></div></div>; }

const Panel = styled(S.SettingSection)`
  --wa-green:#168a45;--wa-soft:#eef8f1;--wa-text:#24211e;--wa-muted:#746e68;gap:18px;
  .wa-header,.wa-title,.channel-state,.profile-actions,.tip{display:flex;align-items:center}.wa-header{justify-content:space-between;gap:18px}.wa-title{gap:13px}.wa-title>span{width:50px;height:50px;border-radius:15px;color:#fff;background:var(--wa-green);display:grid;place-items:center}.wa-title h2{margin:0;color:var(--wa-text);font-size:clamp(24px,2.2vw,31px)}.wa-title p{margin:5px 0 0;color:var(--wa-muted);font-size:11px}
  .channel-state{min-width:225px;justify-content:space-between;gap:14px;border:1px solid #d7e8dc;border-radius:14px;padding:12px 14px;background:#f5fbf7}.channel-state span{display:grid;gap:2px}.channel-state b{color:#176a3c;font-size:11px}.channel-state small{color:#707871;font-size:9px}.switch{appearance:none;width:46px;height:26px;border:0;border-radius:999px;background:#bcc1bc;position:relative;cursor:pointer}.switch::after{content:'';position:absolute;width:20px;height:20px;top:3px;left:3px;border-radius:50%;background:#fff;box-shadow:0 2px 6px rgba(0,0,0,.18);transition:transform 160ms ease}.switch:checked{background:var(--wa-green)}.switch:checked::after{transform:translateX(20px)}.switch:disabled{opacity:.45;cursor:not-allowed}
  ...
  @media(max-width:1050px){.wa-grid{grid-template-columns:1fr}.preview-card{position:static}}@media(max-width:700px){.wa-header{align-items:stretch;flex-direction:column}.channel-state{width:100%}.profile-layout{grid-template-columns:1fr}.profile-preview{justify-items:start}}@media(max-width:520px){.card,.preview-card{padding:15px;border-radius:15px}.wa-title h2{font-size:23px}.automation-row{grid-template-columns:31px minmax(0,1fr)}.automation-row .switch,.badge{grid-column:2;justify-self:start}.chat-preview{padding:10px;max-height:none}}
`;
