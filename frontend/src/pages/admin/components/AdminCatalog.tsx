...
export function AdminCatalog(props: AdminCatalogProps) {
  ...
  const createCategory = async () => {
    ...
    try {
      ...
      setCategoryFeedback(
        errorMessage(error, "Não foi possível criar a categoria."),
      );
    } finally {
  ...
  const renameCategory = async (category: AdminCategory) => {
    const name = await promptDialog({
      ...
      description:
        "Escolha um nome claro para facilitar a organização do cardápio.",
    ...
    try {
      ...
      setCategoryFeedback(
        errorMessage(error, "Não foi possível renomear a categoria."),
      );
    } finally {
      setCategoryBusy(false);
    }
  ...
  const deleteCategory = async (category: AdminCategory) => {
    ...
    try {
      ...
      setCategoryFeedback(
        errorMessage(error, "Não foi possível excluir a categoria."),
      );
    } finally {
      setCategoryBusy(false);
    }
  ...
  return (
    <>
      ...
      <S.Card style={{ marginTop: 24 }}>
        ...
        <p>
          Crie categorias e use as ações ao lado de cada item para renomear ou
          excluir.
        </p>
        {categoryFeedback && (
          <p
            role="alert"
            style={{
              color: categoryFeedback.startsWith("Categoria")
                ? "#166534"
                : "#b91c1c",
            }}
          >
          ...
          {categories.map((category) => (
            <div className="data-row" key={category.id}>
              <div>
                <b>{category.name}</b>
                <span>
                  {countProductsInCategory(products, category.id)} produto(s)
                </span>
              </div>
