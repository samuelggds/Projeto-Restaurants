...
export function BrandSettings({ settings, update, logoInput, onLogoChange, onCoverChange, onEnhanceCover, isEnhancingCover, onBannerChange }: Props) {
  ...
  return <S.Stack>
    <S.Card><S.LogoCard>
      ...
      <div className="logo">{settings.logoUrl ? <img src={settings.logoUrl} alt="Logo" /> : createRestaurantMonogram(settings.restaurantName)}</div>
      <div className="upload"><input hidden ref={logoInput} type="file" accept="image/jpeg,image/png,image/webp" onChange={onLogoChange} /><button onClick={() => logoInput.current?.click()}><Upload />Trocar imagem da marca</button><small>Recomendado: 1600 × 1200 px ou maior,<br />JPG, PNG ou WebP, máximo 5 MB.</small></div>
    </S.LogoCard></S.Card>
    <S.Card><S.LogoCard>
      <div className="copy"><h2>Imagem de capa do acesso</h2><p>Composição quadrada preparada para preencher a metade das telas de acesso sem ampliar demais a marca.</p></div>
      <div className="logo">{settings.coverImageUrl ? <img src={settings.coverImageUrl} alt="Capa do acesso" /> : <ImagePlus />}</div>
      <div className="upload"><input hidden ref={coverInput} type="file" accept="image/jpeg,image/png,image/webp" onChange={onCoverChange} /><button type="button" onClick={() => coverInput.current?.click()}><Upload />Trocar imagem de capa</button><button type="button" disabled={!settings.coverImageUrl || isEnhancingCover} onClick={onEnhanceCover}>{isEnhancingCover ? <LoaderCircle className="spin" /> : <Sparkles />} {isEnhancingCover ? "Melhorando..." : "Melhorar com IA"}</button><small>A IA reconstrói detalhes e prepara a capa em alta definição.<br />A utilização gera custo na conta da API.</small></div>
    </S.LogoCard></S.Card>
    <S.Card><h2>Identidade da marca</h2><S.FormGrid>
      <S.Field $full>Nome do restaurante<S.IdentityNameInput value={settings.restaurantName} onChange={(event) => update("restaurantName", event.target.value)} /></S.Field>
