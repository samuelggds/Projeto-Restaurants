...
const StatusControl = styled.div`
  ...
  button[data-active='true'] {
    ...
    border-color: #9bce9f;
    color: #187137;
    box-shadow: 0 4px 12px rgba(24, 113, 55, 0.12);
  }
  button[data-closed='true'][data-active='true'] {
    background: #fff0ee;
    border-color: #efaaa3;
    color: #bd3c31;
    box-shadow: 0 4px 12px rgba(189, 60, 49, 0.1);
  }
...
export function OpeningHoursSettings({ settings, update }: Props) {
  ...
  return (
    <S.Card>
      ...
      <p>
        Defina os horários regulares e controle na hora se o restaurante está recebendo pedidos.
      </p>
      <StatusControl>
        <div>
          <strong>Status para novos pedidos</strong>
          <small>Altere quando precisar abrir ou fechar o restaurante imediatamente.</small>
        </div>
        <div>
          <button
            type="button"
