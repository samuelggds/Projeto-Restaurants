  ...
  return (
    <S.DemoTopbar>
      <S.DemoTopbarInner>
        ...
        <S.TopActions>
          <S.SoftButton type="button" onClick={onReset}><RefreshCw size={15} /><span className="wide-label">Reiniciar cenário</span></S.SoftButton>
          <S.SoftButton type="button" onClick={onExit}>{loggedIn ? <LogOut size={15} /> : <ArrowLeft size={15} />}<span className="wide-label">{loggedIn ? 'Sair da conta' : 'Voltar'}</span></S.SoftButton>
        </S.TopActions>
      </S.DemoTopbarInner>
    </S.DemoTopbar>
  );
  ...
  return (
    <S.PortalPage>
      <S.PortalHero>
        ...
        <p>As contas já estão prontas. Faça login, movimente pedidos e veja o mesmo cenário pelas diferentes telas do restaurante.</p>
      </S.PortalHero>
      <S.PortalGrid>
        {cards.map((card) => (
          <S.PortalCard key={card.id}>
            <span className="icon">{card.icon}</span><h2>{card.title}</h2><p>{card.text}</p>
            <ul>{card.points.map((point) => <li key={point}><Check size={14} />{point}</li>)}</ul>
            <S.PrimaryButton type="button" onClick={() => onOpen(card.id)}>Entrar nesta área <ArrowRight size={15} /></S.PrimaryButton>
          </S.PortalCard>
        ))}
      </S.PortalGrid>
    </S.PortalPage>
    ...
    <ThemeProvider theme={loginTheme}>
      <LoginS.Container data-testid="demo-login-layout">
        <LoginS.LoginBannerSection $hasLogo={false} data-has-cover="false">
          <TenantBrandHero branding={demoBranding} mode="login" contextLabel={copy.badge} overrideText={copy.hero} />
        </LoginS.LoginBannerSection>
        <LoginS.LoginFormSection>
          <LoginS.LoginFormWrapper>
            <LoginS.LoginAccessBadge><LockKeyhole aria-hidden="true" /><span>{copy.badge}</span></LoginS.LoginAccessBadge>
            ...
            <LoginS.Form onSubmit={submit} autoComplete="off">
              <LoginS.InputGroup><LoginS.Label htmlFor="demo-email">E-mail</LoginS.Label><LoginS.LoginInputField><LoginS.LoginInputIcon aria-hidden="true"><Mail /></LoginS.LoginInputIcon><LoginS.Input id="demo-email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} required /></LoginS.LoginInputField></LoginS.InputGroup>
              <LoginS.InputGroup><LoginS.Label htmlFor="demo-password">Senha</LoginS.Label><LoginS.LoginInputField data-password="true"><LoginS.LoginInputIcon aria-hidden="true"><LockKeyhole /></LoginS.LoginInputIcon><LoginS.Input id="demo-password" type="text" value={password} onChange={(event) => setPassword(event.target.value)} required /></LoginS.LoginInputField></LoginS.InputGroup>
              {error && <p role="alert" style={{ margin: 0, color: '#b42318', fontSize: 13 }}>{error}</p>}
              <LoginS.LoginSubmitButton type="submit"><span>Entrar na demonstração</span><ArrowRight aria-hidden="true" /></LoginS.LoginSubmitButton>
            </LoginS.Form>
            <S.CredentialAside>
              <strong>{accounts.length === 1 ? 'Conta preparada' : 'Contas preparadas'}</strong>
              <small>Clique em uma conta para preencher o login. A senha de todas é {DEMO_DEFAULT_PASSWORD}.</small>
              <S.CredentialList>{accounts.map((account) => <S.CredentialButton key={account.id} type="button" $active={selectedId === account.id} onClick={() => chooseAccount(account)}><span><b>{demoRoleLabels[account.role]}</b><small>{account.email}</small></span><code>{DEMO_DEFAULT_PASSWORD}</code></S.CredentialButton>)}</S.CredentialList>
            </S.CredentialAside>
          </LoginS.LoginFormWrapper>
        </LoginS.LoginFormSection>
      </LoginS.Container>
...
function OrdersPanel({ orders, title, subtitle, actions }: { orders: DemoOrder[]; title: string; subtitle: string; actions?: (order: DemoOrder) => ReactNode }) {
  return (
    <S.Panel>
      <S.PanelHeader><div><h2>{title}</h2><small>{subtitle}</small></div><ClipboardList size={18} /></S.PanelHeader>
      {orders.length ? <S.OrderList>{orders.map((order) => <S.OrderRow key={order.id}><b>{order.publicId}</b><span className="customer"><b>{order.customerName}</b><small>{order.items.map((item) => `${item.quantity}x ${item.name}`).join(' • ')}</small></span><span className="channel">{channelLabel(order)}</span><span className="value"><b>{money(order.total)}</b><small>{order.paid ? 'Pago' : 'A receber'}</small></span><span><S.Status $tone={statusTone(order.status)}>{statusLabels[order.status]}</S.Status>{actions?.(order)}</span></S.OrderRow>)}</S.OrderList> : <S.Empty><ClipboardList size={25} /><b>Nenhum pedido nesta etapa</b><span>Crie ou movimente um pedido para atualizar o cenário.</span></S.Empty>}
    </S.Panel>
  );
}
...
function CustomerWorkspace({ state, onState }: { state: DemoState; onState: (state: DemoState) => void }) {
  ...
  const checkout = () => {
    ...
    onState(result.state);
    setCartOpen(false);
    setNotice(`Pedido ${result.order.publicId} enviado. Agora ele pode ser visto pelas outras áreas.`);
  };

  return (
    <S.CustomerRoot>
      <S.CustomerHeader><div className="brand"><span className="mark">GB</span><span><b>GastroNexa Burger</b><small>Cardápio digital</small></span></div><span className="user">Olá, {account?.name}</span><button className="cart" type="button" onClick={() => setCartOpen(true)} aria-label="Abrir carrinho"><ShoppingBag size={17} />{cartCount || ''}</button></S.CustomerHeader>
      <S.CustomerMain>
        {notice && <S.Toast><CheckCircle2 size={16} />{notice}</S.Toast>}
        <S.CustomerHero><div><span>GastroNexa Burger</span><h1>Sabor feito para o seu momento.</h1><p>Escolha seus favoritos, monte o pedido e acompanhe cada etapa como um cliente do restaurante.</p></div></S.CustomerHero>
        ...
        <div style={{ marginTop: 24 }}><OrdersPanel orders={ownOrders} title="Meus pedidos" subtitle="Pedidos feitos com esta conta demonstrativa" /></div>
      </S.CustomerMain>
      ...
      <S.CartDrawer $open={cartOpen} aria-hidden={!cartOpen}>
        ...
        <S.CartLines>{state.cart.length ? state.cart.map((line) => <S.CartLine key={line.productId}><span><b>{line.name}</b><small>{money(line.unitPrice)} cada</small></span><span className="qty"><button type="button" onClick={() => onState(changeDemoCartQuantity(state, line.productId, line.quantity - 1))}>−</button><b>{line.quantity}</b><button type="button" onClick={() => onState(changeDemoCartQuantity(state, line.productId, line.quantity + 1))}>+</button></span></S.CartLine>) : <S.Empty><ShoppingBag size={24} /><b>Carrinho vazio</b><span>Adicione itens do cardápio.</span></S.Empty>}</S.CartLines>
        <S.Checkout><label>Tipo de pedido<select value={channel} onChange={(event) => setChannel(event.target.value as DemoOrderChannel)}><option value="DELIVERY">Delivery</option><option value="PICKUP">Retirada</option><option value="TABLE">Mesa 08</option></select></label><label>Pagamento<select value={payment} onChange={(event) => setPayment(event.target.value as DemoPaymentMethod)}><option value="PIX">PIX</option><option value="CARD">Cartão</option><option value="CASH">Dinheiro</option></select></label><div className="total"><span>Total</span><strong>{money(getDemoCartTotal(state))}</strong></div><S.PrimaryButton type="button" disabled={!state.cart.length} onClick={checkout}>Enviar pedido <ArrowRight size={15} /></S.PrimaryButton></S.Checkout>
      </S.CartDrawer>
    </S.CustomerRoot>
  );
}
...
function firstView(role: DemoRole) {
  return role === 'MOTOQUEIRO' ? 'ready' : 'overview';
}
...
function MetricsBlock({ state, role }: { state: DemoState; role: DemoRole }) {
  ...
  const configs = role === 'COZINHA'
    ? [['Na fila', state.orders.filter((order) => ['PENDENTE', 'PREPARANDO'].includes(order.status)).length, ChefHat], ['Em preparo', state.orders.filter((order) => order.status === 'PREPARANDO').length, Clock3], ['Prontos', state.orders.filter((order) => order.status === 'PRONTO').length, PackageCheck], ['Concluídos', state.orders.filter((order) => order.status === 'ENTREGUE').length, CheckCircle2]]
    : role === 'MOTOQUEIRO'
      ? [['Prontos', state.orders.filter((order) => order.channel === 'DELIVERY' && order.status === 'PRONTO').length, PackageCheck], ['Em rota', state.orders.filter((order) => order.channel === 'DELIVERY' && order.status === 'SAIU_PARA_ENTREGA').length, Bike], ['Entregues', state.orders.filter((order) => order.channel === 'DELIVERY' && order.status === 'ENTREGUE').length, CheckCircle2], ['Delivery', state.orders.filter((order) => order.channel === 'DELIVERY').length, ShoppingBag]]
      : role === 'GARCOM'
        ? [['Mesas ocupadas', state.tables.filter((table) => table.occupied).length, Store], ['Para entregar', state.orders.filter((order) => order.channel === 'TABLE' && order.status === 'PRONTO').length, ShoppingBag], ['Chamados', state.calls.filter((call) => call.status === 'WAITING').length, BellRing], ['Contas abertas', state.tables.filter((table) => table.occupied).length, CreditCard]]
        : role === 'ATENDENTE'
          ? [['Pedidos ativos', state.orders.filter((order) => !['ENTREGUE', 'CANCELADO'].includes(order.status)).length, ClipboardList], ['Prontos agora', state.orders.filter((order) => order.status === 'PRONTO').length, PackageCheck], ['Chamados', state.calls.filter((call) => call.status !== 'RESOLVED').length, BellRing], ['Mesas ocupadas', state.tables.filter((table) => table.occupied).length, Store]]
          : [['Vendas da demo', money(sales), WalletCards], ['Pedidos', state.orders.length, ShoppingBag], ['Em preparo', state.orders.filter((order) => order.status === 'PREPARANDO').length, Clock3], ['Clientes', new Set(state.orders.map((order) => order.customerEmail)).size, Users]];
  return <S.Metrics>{configs.map(([label, value, Icon]) => <S.Metric key={String(label)}><span className="icon"><Icon size={18} /></span><span><small>{label}</small><b>{value}</b></span></S.Metric>)}</S.Metrics>;
}
...
function OpsWorkspace({ state, onState, onLogout }: { state: DemoState; onState: (state: DemoState) => void; onLogout: () => void }) {
  ...
  return (
    <S.OpsShell>
      <S.Sidebar><S.SidebarBrand><span className="mark">GB</span><b>GastroNexa Burger</b><small>{role === 'ADMIN' ? 'Painel administrativo' : `Área do ${demoRoleLabels[role].toLowerCase()}`}</small></S.SidebarBrand><S.SideNav>{nav.map(([id, label, Icon]) => <button key={id} type="button" className={view === id ? 'active' : ''} onClick={() => setView(id)}><Icon />{label}</button>)}</S.SideNav><S.SideFooter><div className="user"><span className="avatar">{account?.name.slice(0, 2).toUpperCase()}</span><span><b>{account?.name}</b><small>{account?.email}</small></span></div><button type="button" onClick={onLogout}><LogOut size={15} />Sair da conta</button></S.SideFooter></S.Sidebar>
      <S.OpsMain><S.OpsHeader><div><span className="crumb">{demoRoleLabels[role]} / {title}</span><h1>{title}</h1><p>{demoRoleDescriptions[role]}</p></div><span className="status">● Operação demonstrativa</span></S.OpsHeader><S.OpsContent><RoleContent role={role} view={view} state={state} onState={onState} /></S.OpsContent></S.OpsMain>
    </S.OpsShell>
  );
}

export default function InteractiveDemoV3() {
  ...
  return (
    <S.Root>
      <DemoHeader onHome={goHome} onReset={reset} onExit={exit} loggedIn={screen === 'workspace'} />
      {screen === 'portals' && <PortalSelection onOpen={(selected) => { setPortal(selected); setScreen('login'); }} />}
