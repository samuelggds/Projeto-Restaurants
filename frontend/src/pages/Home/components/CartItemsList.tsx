          ...
          <S.CartItemRow key={item.productId}>
            ...
            <S.CartItemInfo>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <S.CartQty>
                  <button type="button" aria-label={`Diminuir ${item.name}`} onClick={() => onDecrease(item.productId)}>−</button>
                  <b>{item.quantity}</b>
                  <button type="button" aria-label={`Aumentar ${item.name}`} onClick={() => onIncrease(item.productId)}>+</button>
                </S.CartQty>
