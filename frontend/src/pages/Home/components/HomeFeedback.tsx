    ...
    <>
      {props.showLoginNudge && (
        <S.LoginNudge>
          ...
          <button className="nudge-login" type="button" onClick={props.onLogin}>Entrar</button>
          <button className="nudge-dismiss" type="button" onClick={props.onDismissNudge}>Agora não</button>
        </S.LoginNudge>
      )}
      <S.NotifStack aria-live="polite">
        {props.notifications.map((notification) => (
          <S.NotifItem key={notification.id} $type={notification.type} $visible={notification.visible}>
            ...
            <button className="notif-close" aria-label="Fechar notificação" type="button" onClick={() => props.onDismissNotification(notification.id)}>×</button>
          </S.NotifItem>
        ))}
      </S.NotifStack>
    </>
