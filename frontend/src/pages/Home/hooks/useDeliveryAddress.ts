...
import customerAddressService, { type CustomerAddress } from "../../../Services/customerAddressService";
import {
...
export function useDeliveryAddress(user: unknown) {
  ...
  const selectSavedAddress = useCallback((address: CustomerAddress, persist = true) => {
    ...
    setDeliveryAddress({ address: address.address, number: address.number, district: address.district, city: address.city, state: address.state, zipCode: formatCep(address.zipCode), complement: String(address.complement || "") });
  ...
  useEffect(() => {
    ...
    let active = true;
    customerAddressService.list().then((items) => {
      if (!active) return;
      setSavedAddresses(items);
      const storedId = localStorage.getItem("selectedCustomerAddressId");
      const selected = items.find((item) => String(item.id) === storedId) || items.find((item) => item.isDefault) || items[0];
