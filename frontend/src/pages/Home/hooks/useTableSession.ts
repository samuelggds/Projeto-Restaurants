...
import { belongsToTableRoute, isTableSessionActive, resolveTableRoute, type StoredTableSession } from "../domain/tableSession";
...
export function useTableSession(options: Options) {
  ...
  const [tableSession, setTableSession] = useState<StoredTableSession | null>(() => readJsonStorage("tableSession", null));
  ...
  const mesaLabel = route.routeTableNumber || activeSession?.tableNumber || activeSession?.tableId || route.routeTableId || "";
  const hasValidQrContext = !route.mesaMode || Boolean(route.routeTableNumber && route.routeTableId);
  const mesaSessionIsActive = isTableSessionActive(activeSession, route.mesaMode, route.routeTableId, route.routeRestaurantId);
  ...
  const handleValidateTablePin = async (event: FormEvent) => {
    ...
    try {
      ...
      setPinError("");
      const result = await tableSessionService.validatePin({ tableId: route.routeTableId, pin: tablePin.trim() });
      ...
      setTablePin("");
      options.notify("success", `Mesa ${next.tableNumber} liberada!`, "Cardápio disponível. Bom apetite!");
    } catch (error) {
  ...
  return { ...route, tablePin, setTablePin, pinError, isPinValidating, mesaLabel, hasValidQrContext, mesaSessionIsActive, storedSessionRestaurantId, handleValidateTablePin };
}
