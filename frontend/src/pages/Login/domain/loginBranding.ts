...
export const DEFAULT_LOGIN_BRANDING: LoginBranding = {
  name: "Peça Já Food", logoUrl: "", primaryColor: "#ef5b00",
};

const RESERVED_ROUTES = new Set(["login", "register", "profile", "admin", "super_admin", "courier", "kitchen", "waiter", "recover-password", "orders"]);
...
export function resolveLoginRestaurant(searchParams: URLSearchParams) {
  ...
  const explicitSlug = String(searchParams.get("restaurantSlug") || searchParams.get("slug") || "").trim().toLowerCase();
  const nextSegment = String(searchParams.get("next") || "").split("?")[0].split("/").filter(Boolean)[0]?.toLowerCase() || "";
...
export function mapLoginBranding(settings: Record<string, unknown> | null): LoginBranding {
  ...
  const logo = restaurant.coverImage || settings.restaurantCoverImage || restaurant.logo || settings.restaurantLogo;
  return {
