  ...
  useEffect(() => {
    ...
    const refreshTracking = () =>
      ordersService
        ...
        .catch((err) =>
          setError(err?.response?.data?.error || 'Não foi possível acompanhar esta entrega.'),
        );
    ...
    const onLocation = (point: RoutePoint & { orderId: number }) => {
      ...
      setData((current) =>
        current ? { ...current, locations: [...current.locations, point].slice(-1000) } : current,
      );
    ...
    const onStatus = (order: TrackingData['order']) => {
      ...
      setData((current) =>
        current ? { ...current, order: { ...current.order, ...order } } : current,
      );
      refreshTracking();
    };
