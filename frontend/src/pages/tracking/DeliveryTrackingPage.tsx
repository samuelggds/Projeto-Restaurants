...
export default function DeliveryTrackingPage() {
  ...
  useEffect(() => {
    ...
    if (!orderId) return;
    ordersService.getDeliveryTracking(orderId)
      .then((result) => setData(result as TrackingData))
    ...
    const onStatus = (order: TrackingData["order"]) => {
      if (order.id === orderId) setData((current) => current ? { ...current, order: { ...current.order, ...order } } : current);
    };
...
const Summary = styled.div`display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;margin-bottom:18px;article{background:#fff;border:1px solid #e5e1dc;border-radius:14px;padding:18px;display:grid;gap:7px}span{font-size:12px;color:#687079}strong{font-size:16px}@media(max-width:700px){grid-template-columns:1fr}`;
