...
export function mapOperationalOrders(raw: unknown[], now = Date.now()): Order[] {
  return (raw as Record<string, unknown>[]).map((record) => {
    ...
    const channel: OrderChannel = type === "MESA" || type === "TABLE_SESSION" ? "TABLE" : type === "DELIVERY" ? "DELIVERY" : "PICKUP";
    ...
    const reference = channel === "TABLE" ? `Mesa ${tableNumber ?? "?"}` : channel === "DELIVERY" ? "Delivery" : "Retirada";
    ...
    return {
      id: `#${record.id}`, channel, reference,
      customer: String((record.user as Record<string, unknown>)?.name || record.customerName || "") || undefined,
      items,
      createdAt: createdAt ? new Date(createdAt).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" }) : "--:--",
      elapsed: createdAt ? formatElapsed(createdAt, now) : "00:00",
      status: String(record.status || "PENDENTE") as OrderStatus,
      total: Number(record.total || 0), observation: String(record.observation || record.notes || "") || undefined,
      completedAt: record.completedAt ? new Date(String(record.completedAt)).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" }) : undefined,
    };
  });
}
