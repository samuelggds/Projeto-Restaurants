...
> = {
  ...
  orders: {
    ...
    subtitle: 'Acompanhe a fila e abra qualquer pedido para entender a situação completa.',
    icon: ClipboardList,
  },
  create: {
    label: 'Novo pedido',
  ...
  support: {
    ...
    subtitle: 'Responda dúvidas e problemas sem misturar o suporte com a cozinha.',
    icon: Headphones,
  },
  deliveries: {
    label: 'Entregas',
    title: 'Acompanhar deliveries',
    subtitle: 'Veja quais entregas estão preparando ou prontas para seguir viagem.',
    icon: Truck,
  },
  tables: {
    label: 'Mesas',
    title: 'Mesas em operação',
    subtitle: 'Entenda rapidamente quais mesas têm pedidos, chamados ou conta solicitada.',
    icon: Armchair,
  },
  calls: {
    label: 'Chamados',
    title: 'Chamados do salão',
    subtitle: 'Assuma uma solicitação para que a equipe saiba quem está cuidando dela.',
    icon: BellRing,
  },
};
...
function PriorityCards({
  ...
  const overdue = snapshot.orders.filter(
    (order) =>
      order.status !== 'PRONTO' &&
      Date.now() - new Date(order.createdAt).getTime() >= 35 * 60_000,
  ).length;
  ...
  return (
    <>
      <S.SectionTitle>
        ...
        <p>Os cards abaixo mostram primeiro o que normalmente exige uma ação do atendente.</p>
      </S.SectionTitle>
      ...
      <S.Guide>
        ...
        <div>
          ...
          <p>
            Comece por pedidos demorando e chamados aguardando. Depois confira os pedidos prontos
            para evitar cliente esperando no balcão ou delivery parado.
          </p>
        </div>
      </S.Guide>
      <S.TwoCols>
        <S.Panel>
          <S.PanelHead>
            <div>
              <BellRing />
              <span><strong>Chamados prioritários</strong><small>Mais antigos aparecem primeiro</small></span>
            </div>
          ...
          {priorityCalls.length ? (
            priorityCalls.map((call) => (
              <S.MiniRow key={call.id}>
                ...
                <time>{elapsed(call.requestedAt)}</time>
              </S.MiniRow>
            ))
          ) : (
        ...
        <S.Panel>
          <S.PanelHead>
            <div>
              <PackageCheck />
              <span><strong>Pedidos prontos</strong><small>Evite deixar pedido pronto parado</small></span>
            </div>
            ...
            readyOrders.map((order) => (
              <S.MiniRow key={order.id}>
                ...
                <span>
                  ...
                  <small>{orderPlace(order)} · {order.items.length} item(ns)</small>
                </span>
                <time>{elapsed(order.readyAt || order.createdAt)}</time>
              </S.MiniRow>
            ))
          ) : (
    ...
    <S.DrawerBackdrop onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <S.Drawer role="dialog" aria-modal="true" aria-label="Detalhes do pedido">
        <S.DrawerHead>
          <div>
            <small>Pedido</small>
            <strong>{fallback?.code || `#${orderId}`}</strong>
            <span>{statusText(currentStatus)}</span>
          </div>
          <button type="button" onClick={onClose} aria-label="Fechar detalhes"><X /></button>
        </S.DrawerHead>

        {loading ? (
          <EmptyState icon={RefreshCw} title="Carregando pedido..." text="Buscando os dados completos deste pedido." />
        ) : (
          <S.DrawerBody>
            <S.InfoGrid>
              <S.Info>
              ...
              <S.Info>
                <small>Canal</small>
                <b>{currentType === 'DELIVERY' ? 'Delivery' : currentType === 'MESA' ? 'Mesa' : 'Retirada'}</b>
            ...
            {currentType === 'DELIVERY' && (
              <S.Info>
                ...
                <b><MapPin /> {String(order.address || 'Endereço não informado')}, {String(order.number || '')}</b>
            ...
            <S.InfoGrid>
              ...
              <S.Info>
                ...
                <b>
                  {canFinishPickup
                    ? paid ? 'Entregar ao cliente' : 'Confirmar pagamento'
                    : currentStatus === 'PRONTO' ? 'Encaminhar para o responsável' : 'Acompanhar preparo'}
                </b>
              </S.Info>
            </S.InfoGrid>
            ...
            {canFinishPickup && (
              <S.ActionBox $warning={!paid}>
                ...
                <p>
                  {paid
                    ? 'Depois de entregar ao cliente, conclua a retirada para remover o pedido da fila ativa.'
                    : 'Por segurança, a retirada só pode ser concluída depois que o pagamento estiver confirmado.'}
                </p>
          ...
          return (
            <S.OrderCard key={order.id} $attention={delayed}>
              <div className="status">
                ...
                <em>{delayed ? 'Precisa de atenção' : statusText(order.status)}</em>
              </div>
              <div className="main">
                <strong>{order.customerName || orderPlace(order)}</strong>
                <small>
                  {orderPlace(order)} · {order.items.map((item) => `${item.quantity}× ${item.productName}`).join(' · ') || 'Itens não informados'}
                </small>
              </div>
              <div className="time"><Clock3 /><span>{elapsed(order.createdAt)}</span></div>
              <button type="button" onClick={() => onOpen(order)}>Ver detalhes <ChevronRight /></button>
            </S.OrderCard>
          );
        })}
        {!visible.length && (
          <EmptyState icon={Search} title="Nenhum pedido encontrado" text="Tente remover algum filtro ou buscar outro termo." />
        )}
      </S.OrderList>
    </>
  );
...
function CreateOrder({ restaurantId, onCreated }: { restaurantId: number; onCreated: () => void }) {
  ...
  const selectedProducts = products.filter((product) => (cart[product.id] || 0) > 0);
  ...
  async function submit(event: FormEvent) {
    ...
    if (customerCpf.replace(/\D/g, '').length !== 11) {
      toast.warning('Informe o CPF do cliente com 11 dígitos.');
      return;
    }
    if (customerPhone.replace(/\D/g, '').length < 10) {
      toast.warning('Informe um telefone com DDD.');
      return;
    }
    ...
    if (
      ...
      toast.warning('Preencha o endereço completo do delivery.');
      return;
    }
    ...
    try {
      ...
      setCustomerCpf('');
      setCustomerPhone('');
      setAddress({ address: '', number: '', district: '', city: '', state: '', zipCode: '', complement: '' });
      onCreated();
    } catch (error) {
    ...
    <S.Form onSubmit={submit}>
      <S.Guide>
        ...
        <div>
          ...
          <p>
            Registre aqui pedidos recebidos por telefone, WhatsApp ou balcão. Pedido de mesa
            continua pela sessão/QR para não perder o vínculo com quem está sentado.
          </p>
        </div>
      </S.Guide>
      ...
      <S.Step>
        ...
        <div>
          ...
          <S.ChoiceRow>
            <button type="button" className={type === 'RETIRADA' ? 'active' : ''} onClick={() => setType('RETIRADA')}>
              <Store /> Retirada no balcão
            </button>
            <button type="button" className={type === 'DELIVERY' ? 'active' : ''} onClick={() => setType('DELIVERY')}>
              <Truck /> Delivery
            </button>
          </S.ChoiceRow>
        </div>
      ...
      <S.Step>
        ...
        <div>
          ...
          <p>Esses dados identificam corretamente o pedido e o histórico.</p>
          <S.FieldGrid>
            <label>Nome<input value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="Ex.: Samuel Gomes" /></label>
            <label>Telefone<input value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value)} placeholder="(85) 99999-9999" /></label>
            <label>CPF<input value={customerCpf} onChange={(e) => setCustomerCpf(e.target.value)} placeholder="000.000.000-00" /></label>
          </S.FieldGrid>
        </div>
      </S.Step>
      ...
      {type === 'DELIVERY' && (
        <S.Step>
          ...
          <div>
            ...
            <p>Revise o endereço com o cliente antes de confirmar.</p>
            <S.FieldGrid>
              <label>Rua<input value={address.address} onChange={(e) => setAddress({ ...address, address: e.target.value })} /></label>
              <label>Número<input value={address.number} onChange={(e) => setAddress({ ...address, number: e.target.value })} /></label>
              <label>Bairro<input value={address.district} onChange={(e) => setAddress({ ...address, district: e.target.value })} /></label>
              <label>Cidade<input value={address.city} onChange={(e) => setAddress({ ...address, city: e.target.value })} /></label>
              <label>UF<input maxLength={2} value={address.state} onChange={(e) => setAddress({ ...address, state: e.target.value.toUpperCase() })} /></label>
              <label>CEP<input value={address.zipCode} onChange={(e) => setAddress({ ...address, zipCode: e.target.value })} /></label>
              <label className="wide">Complemento<input value={address.complement} onChange={(e) => setAddress({ ...address, complement: e.target.value })} /></label>
            </S.FieldGrid>
            <S.ChoiceRow>
              <button type="button" className={payment === 'CARTAO' ? 'active' : ''} onClick={() => setPayment('CARTAO')}>Cartão na entrega</button>
              <button type="button" className={payment === 'PIX' ? 'active' : ''} onClick={() => setPayment('PIX')}>Pix na entrega</button>
            </S.ChoiceRow>
          </div>
        </S.Step>
      )}
      ...
      <S.Step>
        ...
        <div>
          ...
          <p>Use + e − para ajustar a quantidade. Produto sem estoque fica bloqueado.</p>
          <S.Products>
            {products.map((product) => {
              const quantity = cart[product.id] || 0;
              const unavailable = product.stock === 0;
              return (
                <S.ProductRow key={product.id} $disabled={unavailable}>
                  <span>
                    <b>{product.name}</b>
                    <small>{money(product.price)}{unavailable ? ' · Indisponível' : ''}</small>
                  </span>
                  <div>
                    <button type="button" aria-label={`Remover ${product.name}`} disabled={!quantity} onClick={() => changeQuantity(product.id, -1)}>−</button>
                    <strong>{quantity}</strong>
                    <button type="button" aria-label={`Adicionar ${product.name}`} disabled={unavailable} onClick={() => changeQuantity(product.id, 1)}>+</button>
                  </div>
                </S.ProductRow>
              );
            })}
      ...
      <S.Review>
        <span><small>Total estimado</small><strong>{money(total)}</strong><p>{selectedProducts.length} produto(s) selecionado(s)</p></span>
        <button type="submit" disabled={submitting || !selectedProducts.length}>
...
function Calls({
  ...
  async function update(call: AttendantCall, status: 'IN_PROGRESS' | 'RESOLVED') {
    try {
      ...
      toast.success(
        status === 'IN_PROGRESS'
          ? 'Chamado assumido. A equipe agora sabe que você está cuidando dele.'
          : 'Chamado resolvido.',
      );
      onChanged();
    } catch (error) {
        ...
        {visible.map((call) => {
          ...
          return (
            <S.CallCard key={call.id}>
              ...
              <div>
                <strong>{call.type === 'BILL' ? 'Fechamento de conta' : 'Atendimento no salão'}</strong>
                <small>
                  {call.status === 'WAITING'
                    ? 'Ninguém assumiu ainda. Se você for atender, clique em “Assumir”.'
                    : call.status === 'IN_PROGRESS'
                      ? `Em atendimento por ${call.assignedToName || 'equipe'}.`
              ...
              <time>{elapsed(call.requestedAt)}</time>
              {call.status === 'WAITING' && (
                <button type="button" onClick={() => void update(call, 'IN_PROGRESS')}>Assumir chamado</button>
              )}
              {call.status === 'IN_PROGRESS' && mine && (
                <button type="button" className="success" onClick={() => void update(call, 'RESOLVED')}>Marcar como resolvido</button>
              )}
            </S.CallCard>
          );
        })}
        {!visible.length && (
          <EmptyState icon={CheckCircle2} title="Nenhum chamado nesta lista" text="Quando uma mesa pedir ajuda, o chamado aparecerá aqui automaticamente." />
        )}
      </S.OrderList>
    </>
  );
      ...
      {snapshot.tables.map((table) => {
        ...
        return (
          <S.TableCard key={table.id} $attention={attention}>
            <header>
              <span><small>Mesa</small><strong>{String(table.tableNumber).padStart(2, '0')}</strong></span>
              <em>
                {table.status === 'CLOSING_REQUESTED'
                  ? 'Conta solicitada'
                  : table.activeCallCount ? 'Precisa de atenção' : 'Ocupada'}
              </em>
            </header>
            <div>
              <span><Users /> <b>{table.participantCount}</b> pessoas</span>
              <span><ShoppingBag /> <b>{table.activeOrderCount}</b> pedidos</span>
              <span><BellRing /> <b>{table.activeCallCount}</b> chamados</span>
            </div>
            <p>
              {table.status === 'CLOSING_REQUESTED'
                ? 'A mesa pediu o fechamento da conta. Priorize o atendimento.'
                : table.activeCallCount
                  ? 'Existe uma solicitação aberta para esta mesa.'
    ...
    <>
      <S.Guide>
        ...
        <div>
          ...
          <p>
            A cozinha controla o preparo e o motoqueiro controla a entrega. O atendente acompanha
            a situação e orienta o cliente sem avançar etapas que pertencem a outros papéis.
          </p>
        </div>
      </S.Guide>
      <S.OrderList>
        {deliveries.map((order) => (
          <S.OrderCard key={order.id} $attention={order.status === 'PRONTO'}>
            <div className="status"><span>{order.code}</span><em>{statusText(order.status)}</em></div>
            <div className="main">
              <strong>{order.customerName || 'Cliente'}</strong>
              <small>
                {order.status === 'PRONTO'
                  ? 'Pronto para seguir para a etapa de entrega.'
                  : 'Acompanhe o preparo e evite prometer horário sem confirmação.'}
              </small>
            </div>
            <div className="time"><Clock3 /><span>{elapsed(order.createdAt)}</span></div>
        ...
        {!deliveries.length && (
          <EmptyState icon={Truck} title="Nenhum delivery ativo" text="Pedidos de entrega aparecerão aqui enquanto estiverem na operação." />
        )}
      </S.OrderList>
    </>
  );
...
function Support() {
  ...
  const conversations = useMemo(
    () =>
      orders.flatMap((order): SupportSummary[] => {
        ...
        return [{
          orderId,
          customer: String(record(order.user).name || issue.customerName || 'Cliente'),
  ...
  async function send(event: FormEvent) {
    ...
    try {
      ...
      setThread((current) =>
        current
          ? { ...current, messages: Array.isArray(data?.messages) ? data.messages : current.messages }
          : current,
      );
      await load();
    } catch (error) {
  ...
  return (
    <S.SupportLayout>
      <S.Panel>
        <S.PanelHead>
          <div><Headphones /><span><strong>Conversas dos pedidos</strong><small>{conversations.filter((item) => !item.resolved).length} aguardando solução</small></span></div>
          <S.TextButton type="button" onClick={() => void load()}><RefreshCw /> Atualizar</S.TextButton>
        </S.PanelHead>
        <S.SupportList>
          {conversations.map((item) => (
            ...
            >
              <span><b>Pedido #{item.orderId} · {item.customer}</b><small>{item.lastMessage}</small></span>
              <em>{item.resolved ? 'Resolvido' : 'Aberto'}</em>
            </button>
          ))}
          {!conversations.length && (
            <EmptyState icon={CheckCircle2} title="Nenhum atendimento aberto" text="Quando um cliente pedir ajuda pelo pedido, a conversa aparece aqui." />
          )}
        </S.SupportList>
      </S.Panel>
            ...
            <S.PanelHead>
              <div>
                ...
                <span>
                  ...
                  <small>{thread?.customerName || 'Cliente'} · {thread?.orderStatus ? statusText(thread.orderStatus) : 'Carregando'}</small>
                </span>
              </div>
              {thread && !thread.isResolved && (
                <S.TextButton type="button" onClick={() => void resolve()}><CheckCircle2 /> Resolver</S.TextButton>
              )}
            </S.PanelHead>
            <S.Chat>
              {loading ? (
                <EmptyState icon={RefreshCw} title="Carregando conversa..." text="Buscando as mensagens deste pedido." />
              ) : (
                <>
                  {thread?.messages.map((message, index) => {
                    const staff = String(message.senderType || '').toUpperCase() === 'ADMIN';
                    return (
                      <S.Bubble key={String(message.id || index)} $staff={staff}>
                  ...
                  {thread?.isResolved && <S.Resolved><CheckCircle2 /> Atendimento resolvido</S.Resolved>}
                </>
              )}
            </S.Chat>
            {thread && !thread.isResolved && (
              <S.Composer onSubmit={send}>
                <textarea
                  aria-label="Responder cliente"
                  value={draft}
                  onChange={(event) => setDraft(event.target.value.slice(0, 600))}
                  placeholder="Escreva uma resposta curta e clara para o cliente..."
                />
                <button type="submit" disabled={!draft.trim()} aria-label="Enviar resposta"><Send /></button>
              </S.Composer>
            )}
          </>
        ) : (
          <EmptyState icon={Headphones} title="Escolha um atendimento" text="Você verá a conversa e a situação do pedido deste lado." />
        )}
      </S.Panel>
    </S.SupportLayout>
  );
...
export function AttendantOperationCenter({
  attendantId,
  attendantName,
  restaurantId,
  restaurant,
    ...
    <S.Shell style={shellStyle}>
      <S.Sidebar>
        <S.Brand>
          ...
          <div><strong>{restaurant.name}</strong><small>Central do atendente</small></div>
        </S.Brand>
        <S.Nav aria-label="Navegação do atendente">
          {(Object.keys(viewMeta) as AttendantView[]).map((id) => {
            ...
            return (
              <button
                ...
                onClick={() => setView(id)}
              >
                <Icon /><span>{viewMeta[id].label}</span>
              </button>
            );
          })}
        </S.Nav>
        <S.Profile>
          <span>{initials}</span>
          <div><b>{attendantName}</b><small>Atendente</small></div>
          <button type="button" onClick={onLogout} aria-label="Sair"><LogOut /></button>
        </S.Profile>
      </S.Sidebar>
      ...
      <S.Main>
        <S.Topbar>
          ...
          <div className="sync">
            <span className={workspaceState.error ? 'offline' : 'online'}>
              {workspaceState.error ? 'Dados preservados' : 'Operação atualizada'}
            </span>
            <button type="button" onClick={() => void onRefresh()} disabled={workspaceState.refreshing}>
              <RefreshCw /> Atualizar
            </button>
          </div>
        </S.Topbar>
        ...
        {workspaceState.error && (
          <S.ErrorBanner>
            <AlertTriangle />
            <div><b>Não conseguimos atualizar agora</b><span>{workspaceState.error}</span></div>
          </S.ErrorBanner>
        )}

        <S.Content>
          {workspaceState.loading ? (
            <EmptyState icon={RefreshCw} title="Carregando a operação..." text="Buscando pedidos, mesas e chamados." />
          ) : view === 'overview' ? (
            <Overview snapshot={snapshot} onGo={setView} />
          ) : view === 'orders' ? (
            <Orders snapshot={snapshot} onOpen={setSelectedOrder} />
          ) : view === 'create' ? (
            <CreateOrder
            ...
            <Calls calls={snapshot.calls} attendantId={attendantId} onChanged={() => void onRefresh()} />
          )}
        </S.Content>
      </S.Main>
