...
const viewMeta: Record<AttendantView, { label: string; title: string; subtitle: string; icon: LucideIcon }> = {
  ...
  orders: { label: 'Pedidos', title: 'Pedidos em andamento', subtitle: 'Pesquise, filtre e acompanhe a fila sem perder pendências antigas.', icon: ClipboardList },
  ...
  support: { label: 'Atendimento', title: 'Atendimento ao cliente', subtitle: 'Responda dúvidas e problemas sem misturar suporte com cozinha.', icon: Headphones },
  deliveries: { label: 'Entregas', title: 'Acompanhar deliveries', subtitle: 'Acompanhe a situação sem assumir etapas do motoqueiro.', icon: Truck },
  tables: { label: 'Mesas', title: 'Mesas em operação', subtitle: 'Veja rapidamente mesas com pedidos, chamados ou conta solicitada.', icon: Armchair },
  calls: { label: 'Chamados', title: 'Chamados do salão', subtitle: 'Assuma uma solicitação para a equipe saber quem está cuidando dela.', icon: BellRing },
};
...
function isDelayed(order: AttendantOrder, referenceTime: number) {
  ...
  return order.status !== 'PRONTO' && Number.isFinite(created) && referenceTime - created >= 35 * 60_000;
}
...
function EmptyState({ icon: Icon, title, text }: { icon: LucideIcon; title: string; text: string }) {
  return <Empty><Icon /><b>{title}</b><span>{text}</span></Empty>;
}
...
function Overview({ snapshot, onGo }: { snapshot: AttendantWorkspaceSnapshot; onGo: (view: AttendantView) => void }) {
  ...
  const oldCalls = snapshot.calls.filter((call) => call.status !== 'RESOLVED' && pendingDays(call.requestedAt, now) > 0).length;
  const oldTables = snapshot.tables.filter((table) => pendingDays(table.openedAt, now) > 0).length;
  const closingTables = snapshot.tables.filter((table) => table.status === 'CLOSING_REQUESTED').length;
  const cards = [
    { icon: AlertTriangle, label: 'Pendências antigas', value: oldOrders + oldCalls + oldTables, text: 'Itens de dias anteriores que ainda precisam ser resolvidos.', view: 'orders' as AttendantView, tone: 'danger' },
    { icon: Clock3, label: 'Pedidos demorando', value: overdue, text: 'Pedidos acima do tempo de atenção.', view: 'orders' as AttendantView, tone: 'warning' },
    { icon: PackageCheck, label: 'Prontos agora', value: ready, text: 'Pedidos aguardando a próxima etapa.', view: 'orders' as AttendantView, tone: 'success' },
    { icon: BellRing, label: 'Chamados aguardando', value: waitingCalls + closingTables, text: 'Chamados e contas que precisam de atendimento.', view: 'calls' as AttendantView, tone: 'info' },
  ];
  const priorityOrders = [...snapshot.orders].sort((a, b) => a.createdAt.localeCompare(b.createdAt)).slice(0, 5);
  return <>
    <SectionTitle><h2>Precisa da sua atenção agora</h2><p>Pendências antigas aparecem primeiro para nada sumir quando virar o dia.</p></SectionTitle>
    <PriorityGrid>{cards.map(({ icon: Icon, label, value, text, view, tone }) => <PriorityCard key={label} type="button" onClick={() => onGo(view)} $tone={tone}><span className="icon"><Icon /></span><span><small>{label}</small><strong>{value}</strong><p>{text}</p></span><ChevronRight /></PriorityCard>)}</PriorityGrid>
    <Guide><CircleHelp /><div><strong>Ordem sugerida de trabalho</strong><p>Comece pelas pendências de dias anteriores, depois pedidos demorando, chamados e pedidos prontos.</p></div></Guide>
    <Panel><PanelHead><div><ClipboardList /><span><strong>Fila prioritária</strong><small>Mais antigos primeiro</small></span></div><TextButton type="button" onClick={() => onGo('orders')}>Ver todos <ChevronRight /></TextButton></PanelHead>{priorityOrders.length ? priorityOrders.map((order) => <MiniRow key={order.id}><span className="badge">{order.code}</span><span><b>{order.customerName || orderPlace(order)}</b><small>{orderPlace(order)} · {ageLabel(order.createdAt, now)}</small></span><time>{elapsed(order.createdAt, now)}</time></MiniRow>) : <EmptyState icon={CheckCircle2} title="Fila tranquila" text="Nenhum pedido ativo agora." />}</Panel>
  </>;
}

function OrderDrawer({ orderId, fallback, onClose, onCompleted }: { orderId: number; fallback: AttendantOrder; onClose: () => void; onCompleted: () => void }) {
  const [data, setData] = useState<Raw | null>(null);
  const [loading, setLoading] = useState(true);
  const [finishing, setFinishing] = useState(false);
  useEffect(() => {
    let active = true;
    attendantApi.getOrder(orderId).then((value) => { if (active) setData(value); }).catch(() => toast.error('Não foi possível carregar os detalhes do pedido.')).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, [orderId]);
  const order = data || {};
  ...
  return <DrawerBackdrop onMouseDown={(event) => event.target === event.currentTarget && onClose()}><Drawer role="dialog" aria-modal="true" aria-label="Detalhes do pedido"><DrawerHead><div><small>Pedido</small><strong>{fallback.code}</strong><StatusPill>{statusText(status)}</StatusPill></div><button type="button" aria-label="Fechar detalhes" onClick={onClose}><X /></button></DrawerHead>{loading ? <EmptyState icon={RefreshCw} title="Carregando pedido..." text="Buscando os dados completos." /> : <DrawerBody>
    <InfoGrid><Info><small>Cliente</small><b>{String(customer.name || fallback.customerName || 'Cliente')}</b><span>{String(customer.phone || 'Telefone não informado')}</span></Info><Info><small>Canal</small><b>{type === 'DELIVERY' ? 'Delivery' : type === 'MESA' ? 'Mesa' : 'Retirada'}</b><span>{paid ? 'Pagamento confirmado' : 'Pagamento pendente'}</span></Info></InfoGrid>
    {type === 'DELIVERY' && <Info><small>Endereço</small><b><MapPin size={16} /> {String(order.address || 'Endereço não informado')}, {String(order.number || '')}</b><span>{[order.district, order.city, order.state].filter(Boolean).join(' · ')}</span></Info>}
    <OrderItems><h3>Itens do pedido</h3>{items.length ? items.map((item, index) => { const itemData = record(item); const product = record(itemData.product); return <div key={String(itemData.id || index)}><b>{Number(itemData.quantity || 1)}× {String(product.name || itemData.productName || 'Item')}</b>{itemData.observation ? <span>{String(itemData.observation)}</span> : null}</div>; }) : fallback.items.map((item) => <div key={item.productName}><b>{item.quantity}× {item.productName}</b></div>)}</OrderItems>
    <InfoGrid><Info><small>Total</small><b className="big">{money(order.total)}</b></Info><Info><small>Próximo passo</small><b>{canFinishPickup ? paid ? 'Entregar ao cliente' : 'Confirmar pagamento' : status === 'PRONTO' ? 'Encaminhar ao responsável' : 'Acompanhar preparo'}</b></Info></InfoGrid>
    {canFinishPickup && <ActionBox $warning={!paid}><strong>{paid ? 'Pedido pronto para retirada' : 'Pagamento ainda não confirmado'}</strong><p>{paid ? 'Depois de entregar ao cliente, conclua a retirada para remover o pedido da fila ativa.' : 'A retirada só pode ser concluída depois que o pagamento estiver confirmado.'}</p><button type="button" disabled={!paid || finishing} onClick={() => void finishPickup()}><CheckCircle2 /> {finishing ? 'Concluindo...' : 'Confirmar retirada entregue'}</button></ActionBox>}
  </DrawerBody>}</Drawer></DrawerBackdrop>;
}
...
function Orders({ snapshot, onOpen }: { snapshot: AttendantWorkspaceSnapshot; onOpen: (order: AttendantOrder) => void }) {
  ...
  const filtered = snapshot.orders.filter((order) => {
    ...
    const statusMatch = statusFilter === 'ALL' || (statusFilter === 'ATRASADO' ? delayed : order.status === statusFilter);
    const isToday = dateKey(order.createdAt) === dateKey(new Date(now));
    const dayMatch = dayFilter === 'ALL' || (dayFilter === 'TODAY' ? isToday : !isToday);
    const numericCode = order.code.replace(/^#/, '').toLowerCase();
    const haystack = [numericCode, order.code, order.customerName, orderPlace(order), ...order.items.map((item) => item.productName)].filter(Boolean).join(' ').toLowerCase();
  ...
  const changeStatus = (value: string) => { setStatusFilter(value); setPage(0); };
  const changeDay = (value: 'ALL' | 'TODAY' | 'OLD') => { setDayFilter(value); setPage(0); };
  const filters = [['ALL', 'Todos'], ['PENDENTE', 'Novos'], ['PREPARANDO', 'Em preparo'], ['PRONTO', 'Prontos'], ['ATRASADO', 'Atrasados']];
  return <>
    <Toolbar><SearchBox><Search /><input aria-label="Buscar pedidos" value={query} onChange={(event) => changeQuery(event.target.value)} placeholder="Buscar por nº do pedido, cliente, mesa ou item" /></SearchBox><Filters>{filters.map(([value, label]) => <button type="button" key={value} className={statusFilter === value ? 'active' : ''} onClick={() => changeStatus(value)}>{label}</button>)}</Filters></Toolbar>
    <DayFilters><button type="button" className={dayFilter === 'ALL' ? 'active' : ''} onClick={() => changeDay('ALL')}>Todos os dias</button><button type="button" className={dayFilter === 'TODAY' ? 'active' : ''} onClick={() => changeDay('TODAY')}>Hoje</button><button type="button" className={dayFilter === 'OLD' ? 'active old' : ''} onClick={() => changeDay('OLD')}>Pendências anteriores</button><span>{filtered.length} resultado(s)</span></DayFilters>
    <OrderList>{visible.map((order) => { const delayed = isDelayed(order, now); const old = pendingDays(order.createdAt, now) > 0; return <OrderCard key={order.id} $attention={delayed || old}><div className="status"><span>{order.code}</span><em className={old ? 'old' : ''}>{old ? ageLabel(order.createdAt, now) : delayed ? 'Precisa de atenção' : statusText(order.status)}</em></div><div className="main"><strong>{order.customerName || orderPlace(order)}</strong><small>{orderPlace(order)} · {order.items.map((item) => `${item.quantity}× ${item.productName}`).join(' · ') || 'Itens não informados'}</small></div><div className="time"><Clock3 /><span>{elapsed(order.createdAt, now)}</span></div><button type="button" onClick={() => onOpen(order)}>Ver detalhes <ChevronRight /></button></OrderCard>; })}{!visible.length && <EmptyState icon={Search} title="Nenhum pedido encontrado" text="Tente outro número, termo ou filtro." />}</OrderList>
    {filtered.length > PAGE_SIZE && <Pagination><button type="button" disabled={safePage === 0} onClick={() => setPage((current) => Math.max(0, current - 1))}><ChevronLeft /> Voltar 10</button><span>Mostrando {safePage * PAGE_SIZE + 1}–{Math.min((safePage + 1) * PAGE_SIZE, filtered.length)} de {filtered.length}</span><button type="button" disabled={safePage >= pageCount - 1} onClick={() => setPage((current) => Math.min(pageCount - 1, current + 1))}>Próximos 10 <ChevronRight /></button></Pagination>}
  </>;
}
...
function CreateOrder({ restaurantId, onCreated }: { restaurantId: number; onCreated: () => void }) {
  ...
  useEffect(() => {
    let active = true;
    productsService.listProducts(restaurantId).then((values) => {
      if (!active) return;
      const normalized = (Array.isArray(values) ? values : []).flatMap((value: unknown) => {
        ...
        return [{ id, name, price: Number.isFinite(price) ? price : 0, stock: item.stock == null ? null : Number(item.stock), category: String(category.name || item.categoryName || 'Outros').trim() || 'Outros', image: image || null }];
      });
  ...
  function changeQuantity(id: number, delta: number) { setCart((current) => ({ ...current, [id]: Math.max(0, (current[id] || 0) + delta) })); }
  async function submit(event: FormEvent) {
    ...
    if (type === 'DELIVERY' && (!address.address.trim() || !address.number.trim() || !address.district.trim() || !address.city.trim() || address.state.trim().length !== 2 || address.zipCode.replace(/\D/g, '').length !== 8)) return void toast.warning('Preencha o endereço completo do delivery.');
    setSubmitting(true);
    try {
      const payload: Raw = { restaurantId, type, customerName: customerName.trim(), customerCpf: customerCpf.replace(/\D/g, ''), customerPhone: customerPhone.trim(), items: selected.map((product) => ({ productId: product.id, quantity: cart[product.id] })), payOnDelivery: type === 'DELIVERY' };
      if (type === 'DELIVERY') Object.assign(payload, address, { paymentMethod: payment, payOnDeliveryMethod: payment });
      await attendantApi.createOrder(payload);
      toast.success('Pedido registrado. Ele já entrou na operação.');
      onCreated();
    } catch (error) { toast.error(errorMessage(error, 'Não foi possível registrar o pedido.')); } finally { setSubmitting(false); }
  }
  return <CreateForm onSubmit={submit}><Guide><CircleHelp /><div><strong>Quando usar?</strong><p>Pedidos por telefone, WhatsApp ou balcão. Pedido de mesa continua pela sessão/QR.</p></div></Guide>
    ...
    {type === 'DELIVERY' && <Step><span>3</span><div><h3>Para onde vai o pedido?</h3><p>Revise o endereço antes de confirmar.</p><FieldGrid><label>Rua<input value={address.address} onChange={(event) => setAddress({ ...address, address: event.target.value })} /></label><label>Número<input value={address.number} onChange={(event) => setAddress({ ...address, number: event.target.value })} /></label><label>Bairro<input value={address.district} onChange={(event) => setAddress({ ...address, district: event.target.value })} /></label><label>Cidade<input value={address.city} onChange={(event) => setAddress({ ...address, city: event.target.value })} /></label><label>UF<input maxLength={2} value={address.state} onChange={(event) => setAddress({ ...address, state: event.target.value.toUpperCase() })} /></label><label>CEP<input value={address.zipCode} onChange={(event) => setAddress({ ...address, zipCode: event.target.value })} /></label><label className="wide">Complemento<input value={address.complement} onChange={(event) => setAddress({ ...address, complement: event.target.value })} /></label></FieldGrid><PaymentRow><button type="button" className={payment === 'CARTAO' ? 'active' : ''} onClick={() => setPayment('CARTAO')}>Cartão na entrega</button><button type="button" className={payment === 'PIX' ? 'active' : ''} onClick={() => setPayment('PIX')}>Pix na entrega</button></PaymentRow></div></Step>}
    <Step><span>{type === 'DELIVERY' ? '4' : '3'}</span><div><h3>Monte o pedido</h3><p>Produtos organizados por categoria para encontrar mais rápido.</p><CategoryStack>{grouped.map(([category, categoryProducts]) => <CategorySection key={category}><CategoryHead><div><strong>{category}</strong><small>{categoryProducts.length} produto(s)</small></div></CategoryHead><ProductGrid>{categoryProducts.map((product) => { const quantity = cart[product.id] || 0; const unavailable = product.stock === 0; return <ProductCard key={product.id} $disabled={unavailable}><ProductImage>{product.image ? <img src={product.image} alt={product.name} loading="lazy" /> : <ImageOff />}</ProductImage><div className="product-copy"><b>{product.name}</b><small>{money(product.price)}{unavailable ? ' · Indisponível' : ''}</small>{product.stock != null && !unavailable ? <em>{product.stock} em estoque</em> : null}</div><Quantity><button type="button" aria-label={`Remover ${product.name}`} disabled={!quantity} onClick={() => changeQuantity(product.id, -1)}>−</button><strong>{quantity}</strong><button type="button" aria-label={`Adicionar ${product.name}`} disabled={unavailable} onClick={() => changeQuantity(product.id, 1)}>+</button></Quantity></ProductCard>; })}</ProductGrid></CategorySection>)}</CategoryStack></div></Step>
...
function Calls({ calls, attendantId, onChanged }: { calls: AttendantCall[]; attendantId: number; onChanged: () => void }) {
  ...
  async function update(call: AttendantCall, status: 'IN_PROGRESS' | 'RESOLVED') {
    try { await attendantApi.updateCallStatus(call.id, status); toast.success(status === 'IN_PROGRESS' ? 'Chamado assumido.' : 'Chamado resolvido.'); onChanged(); } catch (error) { toast.error(errorMessage(error, 'Não foi possível atualizar o chamado.')); }
  }
  return <><Toolbar><Filters><button type="button" className={mode === 'ACTIVE' ? 'active' : ''} onClick={() => setMode('ACTIVE')}>Aguardando / em atendimento</button><button type="button" className={mode === 'HISTORY' ? 'active' : ''} onClick={() => setMode('HISTORY')}>Resolvidos hoje</button></Filters></Toolbar><OrderList>{visible.map((call) => { const mine = call.assignedToId === attendantId; return <CallCard key={call.id}><span className="table">Mesa {String(call.tableNumber).padStart(2, '0')}</span><div><strong>{call.type === 'BILL' ? 'Fechamento de conta' : 'Atendimento no salão'}</strong><small>{call.status === 'WAITING' ? 'Ninguém assumiu ainda.' : call.status === 'IN_PROGRESS' ? `Em atendimento por ${call.assignedToName || 'equipe'}.` : `Resolvido por ${call.assignedToName || 'equipe'}.`}</small></div><time>{call.status === 'RESOLVED' ? 'Resolvido' : 'Ativo'}</time>{call.status === 'WAITING' && <button type="button" onClick={() => void update(call, 'IN_PROGRESS')}>Assumir chamado</button>}{call.status === 'IN_PROGRESS' && mine && <button type="button" className="success" onClick={() => void update(call, 'RESOLVED')}>Marcar como resolvido</button>}</CallCard>; })}{!visible.length && <EmptyState icon={CheckCircle2} title="Nenhum chamado nesta lista" text="Quando uma mesa pedir ajuda, o chamado aparece aqui." />}</OrderList></>;
}
...
function Tables({ snapshot }: { snapshot: AttendantWorkspaceSnapshot }) {
  ...
  return <TableGrid>{snapshot.tables.map((table) => { const attention = table.status === 'CLOSING_REQUESTED' || table.activeCallCount > 0 || pendingDays(table.openedAt, now) > 0; return <TableCard key={table.id} $attention={attention}><header><span><small>Mesa</small><strong>{String(table.tableNumber).padStart(2, '0')}</strong></span><em>{pendingDays(table.openedAt, now) > 0 ? ageLabel(table.openedAt, now) : table.status === 'CLOSING_REQUESTED' ? 'Conta solicitada' : table.activeCallCount ? 'Precisa de atenção' : 'Ocupada'}</em></header><div><span><Users /> <b>{table.participantCount}</b> pessoas</span><span><ShoppingBag /> <b>{table.activeOrderCount}</b> pedidos</span><span><BellRing /> <b>{table.activeCallCount}</b> chamados</span></div><p>{attention ? 'Esta mesa merece atenção operacional.' : 'Mesa sem pendências sinalizadas agora.'}</p></TableCard>; })}</TableGrid>;
}
...
function Deliveries({ snapshot, onOpen }: { snapshot: AttendantWorkspaceSnapshot; onOpen: (order: AttendantOrder) => void }) {
  ...
  return <><Guide><Truck /><div><strong>O que acompanhar aqui</strong><p>A cozinha controla o preparo e o motoqueiro controla a entrega. O atendente acompanha e orienta o cliente.</p></div></Guide><OrderList>{deliveries.map((order) => <OrderCard key={order.id} $attention={order.status === 'PRONTO' || pendingDays(order.createdAt, now) > 0}><div className="status"><span>{order.code}</span><em>{pendingDays(order.createdAt, now) > 0 ? ageLabel(order.createdAt, now) : statusText(order.status)}</em></div><div className="main"><strong>{order.customerName || 'Cliente'}</strong><small>{order.status === 'PRONTO' ? 'Pronto para seguir para a etapa de entrega.' : 'Acompanhe o preparo sem prometer horário sem confirmação.'}</small></div><div className="time"><Clock3 /><span>{elapsed(order.createdAt, now)}</span></div><button type="button" onClick={() => onOpen(order)}>Ver pedido <ChevronRight /></button></OrderCard>)}{!deliveries.length && <EmptyState icon={Truck} title="Nenhum delivery ativo" text="Pedidos de entrega aparecem aqui enquanto estiverem na operação." />}</OrderList></>;
}
...
function Support() {
  ...
  const conversations = useMemo(() => orders.flatMap((order): SupportSummary[] => { const issue = record(order.issueThread); if (!Object.keys(issue).length) return []; const messages = Array.isArray(issue.messages) ? issue.messages : []; const last = messages.length ? record(messages[messages.length - 1]) : {}; const orderId = Number(order.id || issue.orderId); if (!Number.isSafeInteger(orderId) || orderId <= 0) return []; return [{ orderId, customer: String(record(order.user).name || issue.customerName || 'Cliente'), lastMessage: String(last.message || 'Atendimento iniciado'), resolved: Boolean(issue.isResolved) }]; }), [orders]);
  ...
  return <SupportLayout><Panel><PanelHead><div><Headphones /><span><strong>Conversas dos pedidos</strong><small>{conversations.filter((item) => !item.resolved).length} aguardando solução</small></span></div><TextButton type="button" onClick={() => void load()}><RefreshCw /> Atualizar</TextButton></PanelHead><SupportList>{conversations.map((item) => <button type="button" key={item.orderId} className={selected === item.orderId ? 'active' : ''} onClick={() => void open(item.orderId)}><span><b>Pedido #{item.orderId} · {item.customer}</b><small>{item.lastMessage}</small></span><em>{item.resolved ? 'Resolvido' : 'Aberto'}</em></button>)}{!conversations.length && <EmptyState icon={CheckCircle2} title="Nenhum atendimento aberto" text="Quando um cliente pedir ajuda pelo pedido, a conversa aparece aqui." />}</SupportList></Panel><Panel>{selected ? <><PanelHead><div><Headphones /><span><strong>Pedido #{selected}</strong><small>{thread?.customerName || 'Cliente'} · {thread?.orderStatus ? statusText(thread.orderStatus) : 'Carregando'}</small></span></div>{thread && !thread.isResolved && <TextButton type="button" onClick={() => void resolve()}><CheckCircle2 /> Resolver</TextButton>}</PanelHead><Chat>{loading ? <EmptyState icon={RefreshCw} title="Carregando conversa..." text="Buscando as mensagens." /> : <>{thread?.messages.map((message, index) => { const staff = String(message.senderType || '').toUpperCase() === 'ADMIN'; return <Bubble key={String(message.id || index)} $staff={staff}><b>{staff ? 'Restaurante' : message.senderName || 'Cliente'}</b><p>{message.message}</p>{message.sentAt ? <time>{new Date(message.sentAt).toLocaleString('pt-BR')}</time> : null}</Bubble>; })}{thread?.isResolved && <Resolved><CheckCircle2 /> Atendimento resolvido</Resolved>}</>}</Chat>{thread && !thread.isResolved && <Composer onSubmit={send}><textarea aria-label="Responder cliente" value={draft} onChange={(event) => setDraft(event.target.value.slice(0, 600))} placeholder="Escreva uma resposta curta e clara..." /><button type="submit" disabled={!draft.trim()} aria-label="Enviar resposta"><Send /></button></Composer>}</> : <EmptyState icon={Headphones} title="Escolha um atendimento" text="Você verá a conversa e a situação do pedido deste lado." />}</Panel></SupportLayout>;
}

export function AttendantOperationCenterV2({ attendantId, attendantName, restaurantId, restaurant, snapshot, workspaceState, onRefresh, onLogout }: Props) {
  ...
  return <Shell style={shellStyle}><Sidebar><Brand><span>{restaurant.monogram}</span><div><strong>{restaurant.name}</strong><small>Central do atendente</small></div></Brand><Nav aria-label="Navegação do atendente">{(Object.keys(viewMeta) as AttendantView[]).map((id) => { const Icon = viewMeta[id].icon; return <button type="button" key={id} className={view === id ? 'active' : ''} onClick={() => setView(id)}><Icon /><span>{viewMeta[id].label}</span></button>; })}</Nav><Profile><span>{initials}</span><div><b>{attendantName}</b><small>Atendente</small></div><button type="button" onClick={onLogout} aria-label="Sair"><LogOut /></button></Profile></Sidebar><Main><Topbar><div><span className="eyebrow">Operação em tempo real</span><h1>{meta.title}</h1><p>{meta.subtitle}</p></div><div className="sync"><span className={workspaceState.error ? 'offline' : 'online'}>{workspaceState.error ? 'Dados preservados' : 'Operação atualizada'}</span><button type="button" onClick={() => void onRefresh()} disabled={workspaceState.refreshing}><RefreshCw /> Atualizar</button></div></Topbar>{workspaceState.error && <ErrorBanner><AlertTriangle /><div><b>Não conseguimos atualizar agora</b><span>{workspaceState.error}</span></div></ErrorBanner>}<Content>{workspaceState.loading ? <EmptyState icon={RefreshCw} title="Carregando a operação..." text="Buscando pedidos, mesas e chamados." /> : view === 'overview' ? <Overview snapshot={snapshot} onGo={setView} /> : view === 'orders' ? <Orders snapshot={snapshot} onOpen={setSelectedOrder} /> : view === 'create' ? <CreateOrder restaurantId={restaurantId} onCreated={() => { void onRefresh(); setView('orders'); }} /> : view === 'support' ? <Support /> : view === 'deliveries' ? <Deliveries snapshot={snapshot} onOpen={setSelectedOrder} /> : view === 'tables' ? <Tables snapshot={snapshot} /> : <Calls calls={snapshot.calls} attendantId={attendantId} onChanged={() => void onRefresh()} />}</Content></Main>{selectedOrder && <OrderDrawer orderId={selectedOrder.orderId} fallback={selectedOrder} onClose={() => setSelectedOrder(null)} onCompleted={() => void onRefresh()} />}</Shell>;
}

const Shell = styled.div`--brand:#e16a3d;min-height:100dvh;display:grid;grid-template-columns:280px minmax(0,1fr);background:#f5f7f8;color:#18231d;font-family:Inter,system-ui,sans-serif;@media(max-width:900px){grid-template-columns:1fr;padding-bottom:84px;}`;
const Sidebar = styled.aside`position:sticky;top:0;height:100dvh;padding:28px 20px;display:flex;flex-direction:column;background:#153729;color:#fff;z-index:20;@media(max-width:900px){position:fixed;inset:auto 0 0;width:100%;height:78px;padding:8px 6px;display:block;}`;
const Brand = styled.div`display:flex;align-items:center;gap:12px;padding:2px 6px 24px;border-bottom:1px solid rgba(255,255,255,.1);>span{width:44px;height:44px;display:grid;place-items:center;border-radius:13px;background:var(--brand);font-weight:900;font-size:16px}strong,small{display:block}strong{font-size:15px}small{margin-top:3px;font-size:11px;opacity:.68}@media(max-width:900px){display:none;}`;
const Nav = styled.nav`display:grid;gap:8px;margin-top:24px;button{min-height:52px;padding:0 15px;display:flex;align-items:center;gap:12px;border:0;border-radius:13px;background:transparent;color:rgba(255,255,255,.72);font-size:13.5px;font-weight:800;cursor:pointer;text-align:left}button svg{width:20px}button:hover,button.active{background:rgba(255,255,255,.1);color:#fff}button.active{box-shadow:inset 3px 0 var(--brand)}@media(max-width:900px){display:flex;gap:4px;height:62px;margin:0;overflow-x:auto;button{min-width:82px;min-height:60px;padding:4px 7px;flex:1;flex-direction:column;justify-content:center;gap:4px;font-size:10px}button svg{width:20px}button.active{box-shadow:inset 0 3px var(--brand)}}`;
const Profile = styled.div`margin-top:auto;padding:18px 6px 0;display:grid;grid-template-columns:40px minmax(0,1fr) 36px;gap:10px;align-items:center;border-top:1px solid rgba(255,255,255,.1);>span{width:40px;height:40px;display:grid;place-items:center;border-radius:50%;background:rgba(255,255,255,.12);font-size:12px;font-weight:900}b,small{display:block}b{overflow:hidden;font-size:12px;white-space:nowrap;text-overflow:ellipsis}small{font-size:10px;opacity:.6}button{border:0;background:transparent;color:#fff;cursor:pointer}button svg{width:19px}@media(max-width:900px){display:none;}`;
const Main = styled.main`min-width:0;`;
const Topbar = styled.header`padding:32px clamp(24px,3vw,56px) 24px;display:flex;justify-content:space-between;gap:28px;align-items:flex-end;background:#fff;border-bottom:1px solid #e8ece9;.eyebrow{font-size:11px;letter-spacing:.12em;text-transform:uppercase;color:#6f7e75;font-weight:900}h1{margin:6px 0;font-size:clamp(32px,2.5vw,42px);letter-spacing:-.04em}p{margin:0;color:#69776f;font-size:14px}.sync{display:flex;align-items:center;gap:12px}.sync>span{font-size:12px;font-weight:800}.online{color:#2f7a4c}.offline{color:#ad6b17}.sync button{min-height:46px;padding:0 16px;display:flex;align-items:center;gap:7px;border:1px solid #dce4df;border-radius:12px;background:#fff;font-size:12px;font-weight:800;cursor:pointer}@media(max-width:700px){padding:22px 18px 18px;align-items:flex-start;flex-direction:column;gap:16px;h1{font-size:30px}.sync{width:100%;justify-content:space-between}}`;
const Content = styled.div`width:100%;max-width:none;margin:0;padding:28px clamp(20px,3vw,56px) 64px;@media(max-width:700px){padding:20px 16px 42px;}`;
...
const PriorityCard = styled.button<{ $tone:string }>`padding:20px;display:grid;grid-template-columns:48px 1fr 20px;gap:13px;align-items:start;border:1px solid ${({$tone})=>$tone==='danger'?'#f0c9c4':$tone==='warning'?'#edd7a8':$tone==='success'?'#c9e5d3':'#caddea'};border-radius:20px;background:#fff;box-shadow:0 8px 24px rgba(32,55,43,.05);text-align:left;cursor:pointer;.icon{width:48px;height:48px;display:grid;place-items:center;border-radius:14px;background:#f3f8f5}.icon svg{width:22px}small,strong{display:block}small{font-size:11px;font-weight:900;text-transform:uppercase;color:#738078}strong{font-size:30px;margin:2px 0}p{margin:0;color:#6d7972;font-size:13px;line-height:1.45}>svg{width:18px;margin-top:13px;color:#9aa59f}`;
...
const SearchBox = styled.label`min-width:320px;min-height:48px;padding:0 14px;flex:1;display:flex;align-items:center;gap:9px;border:1px solid #dce4df;border-radius:13px;background:#fff;svg{width:18px;color:#7c8981}input{flex:1;border:0;outline:0;background:transparent;font-size:14px}@media(max-width:600px){min-width:100%}`;
...
const OrderList = styled.div`display:grid;gap:11px`;
const OrderCard = styled.article<{ $attention?:boolean }>`padding:16px 18px;display:grid;grid-template-columns:130px minmax(0,1fr) 100px auto;gap:14px;align-items:center;border:1px solid ${({$attention})=>$attention?'#e8c69a':'#e1e7e3'};border-radius:16px;background:#fff;box-shadow:0 5px 18px rgba(30,50,40,.035);.status span,.status em,.main strong,.main small{display:block}.status span{font-size:16px;font-weight:900}.status em{margin-top:3px;color:#4f785e;font-size:11px;font-style:normal}.status em.old{color:#a06417}.main strong{font-size:14px}.main small{margin-top:4px;overflow:hidden;color:#77847c;font-size:12px;white-space:nowrap;text-overflow:ellipsis}.time{display:flex;align-items:center;gap:6px;color:#7d8982;font-size:12px}.time svg{width:15px}>button{min-height:42px;padding:0 13px;display:flex;align-items:center;gap:5px;border:1px solid #d8e2dc;border-radius:11px;background:#f8fbf9;color:#315e45;font-size:12px;font-weight:850;cursor:pointer}@media(max-width:760px){grid-template-columns:90px 1fr;padding:15px;.time{display:none}.main small{white-space:normal}>button{grid-column:1/-1;justify-content:center}}`;
...
const DrawerHead = styled.header`padding:24px 24px 20px;display:flex;justify-content:space-between;align-items:flex-start;border-bottom:1px solid #e2e8e4;background:#fff;small,strong{display:block}small{color:#7b8980;font-size:11px;text-transform:uppercase}strong{font-size:30px}button{width:44px;height:44px;border:0;border-radius:12px;background:#f3f6f4;cursor:pointer}`;
const StatusPill = styled.span`display:inline-flex!important;width:max-content;margin-top:6px;padding:6px 9px;border-radius:999px;background:#edf8f1;color:#34704d;font-size:11px;font-weight:850`;
...
const Info = styled.div`padding:15px;border:1px solid #e2e8e4;border-radius:14px;background:#fff;small,b,span{display:block}small{color:#7c8981;font-size:10px;font-weight:850;text-transform:uppercase}b{margin-top:5px;font-size:14px;display:flex;align-items:center;gap:5px}b.big{font-size:21px}span{margin-top:4px;color:#76837b;font-size:11px}`;
const OrderItems = styled.div`overflow:hidden;border:1px solid #e2e8e4;border-radius:14px;background:#fff;h3{margin:0;padding:14px 15px;border-bottom:1px solid #edf1ee;font-size:14px}>div{padding:12px 15px;border-bottom:1px solid #edf1ee}>div:last-child{border-bottom:0}b,span{display:block}b{font-size:13px}span{margin-top:3px;color:#77847c;font-size:11px}`;
...
const CategoryStack = styled.div`display:grid;gap:22px`;
const CategorySection = styled.section`padding:0!important;border:0!important;background:transparent!important`;
const CategoryHead = styled.div`margin-bottom:10px;display:flex;align-items:end;justify-content:space-between;strong,small{display:block}strong{font-size:15px}small{margin-top:2px;color:#78857d;font-size:11px}`;
...
const ProductCard = styled.div<{ $disabled?:boolean }>`min-height:108px;padding:10px;display:grid;grid-template-columns:82px minmax(0,1fr) auto;gap:12px;align-items:center;border:1px solid #e1e7e3;border-radius:14px;background:#fff;opacity:${({$disabled})=>$disabled?.55:1};.product-copy b,.product-copy small,.product-copy em{display:block}.product-copy b{font-size:13px}.product-copy small{margin-top:3px;color:#66746c;font-size:11px}.product-copy em{margin-top:5px;color:#4f785e;font-size:10px;font-style:normal}@media(max-width:420px){grid-template-columns:70px minmax(0,1fr);>div:last-child{grid-column:1/-1;justify-self:end}}`;
