...
if (!mode || !code || !['notification', 'transaction'].includes(mode)) {
  console.error('Uso: node scripts/testPagBankTransaction.mjs <notification|transaction> <code>');
  process.exit(1);
}
...
(async () => {
  try {
    ...
    console.log(`[PAGBANK_TRANSACTION_TEST] GET ${endpoint.replace(pagBankToken, '***')}`);
