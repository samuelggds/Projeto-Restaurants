...
async function sendEmailAlert(title, details) {
    ...
    try {
        ...
        console.error("[ALERT_EMAIL_ERROR]", emailError instanceof Error ? emailError.message : String(emailError));
        return false;
    }
}
...
export async function notifyCriticalError(title, details) {
    ...
    try {
        ...
        console.error("[ALERT_WEBHOOK_ERROR]", notificationError instanceof Error
