...
class RequestPlanChangeService {
    async execute({ restaurantId, plan }) {
        ...
        if (!latestInvoice || latestInvoice.status !== "PAGO") {
            throw new Error("Para trocar de plano, a ultima fatura precisa estar paga.");
        }
        const hasOpenInvoice = invoices.some((invoice) => ["PENDENTE", "ATRASADO", "VENCIDO"].includes(String(invoice.status || "").toUpperCase()));
        if (hasOpenInvoice) {
            throw new Error("Regularize as faturas pendentes para liberar a troca de plano.");
        }
        ...
        if (now > planChangeDeadline) {
            throw new Error(`A troca de plano so pode ser solicitada em ate 30 dias apos o pagamento da fatura. Prazo encerrado em ${planChangeDeadline.toLocaleDateString("pt-BR")}.`);
        }
        if (subscription.planChangeLockedUntil &&
            now < new Date(subscription.planChangeLockedUntil)) {
            throw new Error(`Voce ja solicitou uma troca. Nova alteracao disponivel apos ${new Date(subscription.planChangeLockedUntil).toLocaleDateString("pt-BR")}.`);
        }
        if (subscription.plan === plan) {
            throw new Error("Esse ja e o plano atual da assinatura.");
        }
        if (subscription.scheduledPlan) {
            throw new Error("Ja existe uma troca de plano agendada para o proximo ciclo.");
        }
