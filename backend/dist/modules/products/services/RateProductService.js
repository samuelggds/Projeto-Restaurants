...
class RateProductService {
    async execute({ productId, restaurantId, clientKey, rating, }) {
        ...
        const product = await productRepository.findById(normalizedProductId, normalizedRestaurantId);
        if (!product) {
        ...
        const summary = await productRepository.getRatingSummary(normalizedProductId, normalizedRestaurantId, normalizedClientKey);
        return {
            rating: summary,
        };
    }
