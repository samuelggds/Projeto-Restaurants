...
class OnboardRestaurantAsaasService {
    ...
    async execute({ restaurantId, cnpj, cpf, restaurantName, pixKey, }) {
        ...
        const existingSettings = await restaurantSettingsRepository.findByRestaurantId(normalizedRestaurantId);
        if (existingSettings) {
