...
class UpdateRestaurantSettingsService {
    ...
    async execute({ restaurantId, deliveryFee, minimumOrder, pixProvider, pixKey, legalDocumentType, companyDocument, companyLegalName, companyTradeName, companyAddress, companyCnae, monthlyRevenue, ownerFullName, ownerCpf, ownerBirthDate, ownerEmail, ownerPhone, ownerAddress, bankName, bankCode, bankAccountType, bankBranch, bankAccount, bankHolderDocument, cardGateway, gatewayMerchantId, stripeSecretKey, mercadoPagoAccessToken, picpayToken, asaasAccessToken, pagbankEmail, pagbankToken, pagbankEnvironment, ownerDocumentFileUrl, bankProofFileUrl, companyContractFileUrl, whatsapp, instagram, facebook, restaurantName, restaurantLogo, restaurantCoverImage, }) {
        const settings = await restaurantSettingsRepository.findByRestaurantId(restaurantId);
        if (!settings) {
            throw new Error("Configurações não encontradas!");
        }
        const normalizedWhatsapp = whatsapp === undefined
            ? undefined
            : String(whatsapp || "").trim() || null;
        const normalizedRestaurantName = restaurantName === undefined
            ? undefined
            : String(restaurantName || "").trim();
        const normalizedRestaurantLogo = restaurantLogo === undefined
            ? undefined
            : String(restaurantLogo || "").trim() || null;
        const normalizedRestaurantCoverImage = restaurantCoverImage === undefined
            ? undefined
            : String(restaurantCoverImage || "").trim() || null;
        const normalizedBankName = bankName === undefined
            ? undefined
            : String(bankName || "").trim() || null;
        const normalizedBankBranch = bankBranch === undefined
            ? undefined
            : String(bankBranch || "").trim() || null;
        const normalizedBankAccount = bankAccount === undefined
            ? undefined
            : String(bankAccount || "").trim() || null;
        const normalizedCardGateway = cardGateway === undefined
            ? undefined
            : String(cardGateway || "").trim() || null;
        const normalizedGatewayMerchantId = gatewayMerchantId === undefined
            ? undefined
            : String(gatewayMerchantId || "").trim() || null;
        const normalizedStripeSecretKey = stripeSecretKey === undefined
            ? undefined
            : String(stripeSecretKey || "").trim() || null;
        const normalizedMercadoPagoAccessToken = mercadoPagoAccessToken === undefined
            ? undefined
            : String(mercadoPagoAccessToken || "").trim() || null;
        const normalizedPicPayToken = picpayToken === undefined
            ? undefined
            : String(picpayToken || "").trim() || null;
        const normalizedAsaasAccessToken = asaasAccessToken === undefined
            ? undefined
            : String(asaasAccessToken || "").trim() || null;
        const normalizedPagBankEmail = pagbankEmail === undefined
            ? undefined
            : String(pagbankEmail || "").trim() || null;
        const normalizedPagBankToken = pagbankToken === undefined
            ? undefined
            : String(pagbankToken || "").trim() || null;
        const normalizedPagBankEnvironment = "production";
        const normalizedLegalDocumentType = legalDocumentType === undefined
            ? undefined
            : String(legalDocumentType || "")
                .trim()
                .toUpperCase() || null;
        const normalizedCompanyDocument = companyDocument === undefined
            ? undefined
            : String(companyDocument || "").replace(/\D/g, "") || null;
        const normalizedOwnerCpf = ownerCpf === undefined
            ? undefined
            : String(ownerCpf || "").replace(/\D/g, "") || null;
        const normalizedOwnerPhone = ownerPhone === undefined
            ? undefined
            : String(ownerPhone || "").replace(/\D/g, "") || null;
        const normalizedBankHolderDocument = bankHolderDocument === undefined
            ? undefined
            : String(bankHolderDocument || "").replace(/\D/g, "") || null;
        const normalizedOwnerBirthDate = ownerBirthDate === undefined
            ? undefined
            : ownerBirthDate
                ? new Date(ownerBirthDate)
                : null;
        const resolvedAsaasToken = normalizedAsaasAccessToken === undefined
        ...
        const resolvedPixProvider = String(pixProvider || settings.pixProvider || "MERCADO_PAGO")
            .trim()
            .toUpperCase();
        const resolvedCardGateway = normalizedCardGateway === undefined
            ...
            : String(normalizedCardGateway || "")
                .trim()
                .toUpperCase();
        let resolvedGatewayMerchantId = normalizedGatewayMerchantId === undefined
        ...
        const shouldTryAutoResolveGatewayMerchantId = !resolvedGatewayMerchantId &&
        ...
        if (resolvedCompanyDocument &&
            resolvedBankHolderDocument &&
            resolvedCompanyDocument !== resolvedBankHolderDocument) {
            throw new Error("A titularidade da conta bancária deve ser igual ao documento cadastrado (CPF/CNPJ).");
        }
        const updated = await restaurantSettingsRepository.update(restaurantId, {
            ...
            companyCnae: companyCnae === undefined
                ? undefined
            ...
            ownerDocumentFileUrl: ownerDocumentFileUrl === undefined
                ? undefined
                : String(ownerDocumentFileUrl || "").trim() || null,
            bankProofFileUrl: bankProofFileUrl === undefined
                ? undefined
                : String(bankProofFileUrl || "").trim() || null,
            companyContractFileUrl: companyContractFileUrl === undefined
                ? undefined
        ...
        return {
            ...
            mercadoPagoAccessTokenConfigured: Boolean(String(updated?.mercadoPagoAccessToken || "").trim()),
            ...
            restaurantCoverImage: restaurantCoverImage !== undefined
