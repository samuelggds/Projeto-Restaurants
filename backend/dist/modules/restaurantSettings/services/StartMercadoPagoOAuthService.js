...
class StartMercadoPagoOAuthService {
    ...
    getAuthBaseUrl() {
        return String(process.env.MP_OAUTH_AUTH_URL ||
    ...
    async execute({ restaurantId, userId }) {
        ...
        if (!clientId) {
            throw new Error("Client ID OAuth do Mercado Pago nao configurado. Defina MP_OAUTH_CLIENT_ID (ou MP_CLIENT_ID / MERCADO_PAGO_CLIENT_ID) no backend.");
        }
