const MEMORY_TTL_MS = Number(process.env.AI_CHAT_MEMORY_TTL_MS || 30 * 60 * 1000);
const MEMORY_MAX_TURNS = Number(process.env.AI_CHAT_MEMORY_MAX_TURNS || 12);
const MEMORY_MAX_CONVERSATIONS = Number(process.env.AI_CHAT_MEMORY_MAX_CONVERSATIONS || 1500);
...
const SECURITY_REFUSAL_MESSAGE = "Nao posso ajudar com dados sensiveis, credenciais, detalhes internos ou instrucoes que comprometam a seguranca. Posso orientar apenas fluxos seguros de uso da plataforma.";
const ROUTE_TIPS = [
    {
        test: (pathname) => pathname === "/" || pathname === "/menu" || pathname === "/cardapio",
    ...
    {
        test: (pathname) => pathname.startsWith("/courier"),
    ...
    {
        test: (pathname) => pathname.startsWith("/billing"),
        title: "Faturamento",
        help: "Consulte faturas, vencimentos e cobranca.",
    },
    {
        test: (pathname) => pathname.startsWith("/super_admin"),
        title: "Painel super admin",
        help: "Monitore licencas, restaurantes e visao global.",
    },
    {
        test: (pathname) => pathname.startsWith("/mesa/"),
...
function pruneExpiredConversations() {
    ...
    const sortedByAge = Array.from(memoryStore.entries()).sort((a, b) => a[1].updatedAt - b[1].updatedAt);
    const overflow = memoryStore.size - MEMORY_MAX_CONVERSATIONS;
    for (let index = 0; index < overflow; index += 1) {
        memoryStore.delete(sortedByAge[index][0]);
    }
}
function getConversationTurns(conversationId) {
...
function saveConversationTurn(conversationId, question, answer) {
    const entry = memoryStore.get(conversationId) || {
        turns: [],
        updatedAt: Date.now(),
    };
...
function buildFallbackAnswer(question, role, pathname) {
    const normalizedQuestion = normalizeText(question);
    const route = getRouteContext(pathname);
    if (normalizedQuestion.includes("oi") ||
    ...
    if (normalizedQuestion.includes("o que voce sabe") ||
    ...
    if (normalizedQuestion.includes("pedido") ||
    ...
    if (normalizedQuestion.includes("pagamento") ||
    ...
    if (normalizedQuestion.includes("perfil") ||
    ...
    if (normalizedQuestion.includes("matematica") ||
    ...
    if (normalizedQuestion.includes("programacao") ||
...
async function askOpenAi(question, role, pathname, history) {
...
async function askGemini(question, role, pathname, history) {
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey) {
        return null;
    }
    const baseUrl = String(process.env.GEMINI_BASE_URL ||
        "https://generativelanguage.googleapis.com/v1").replace(/\/+$/, "");
...
async function askWithProviders(question, role, pathname, history) {
...
class AiSupportService {
    async execute(payload) {
        ...
        try {
            const providerResult = await askWithProviders(question, role, pathname, mergedHistory);
            if (providerResult?.answer) {
                if (PROHIBITED_PATTERNS.some((pattern) => pattern.test(providerResult.answer))) {
                    return {
                        answer: SECURITY_REFUSAL_MESSAGE,
                        blocked: true,
                        source: "fallback",
