...
import { getOrderIssueThread, resolveOrderIssueThread, toOrderIssueThreadPayload, } from "./orderIssueChatStore.js";
const REFUND_REQUEST_PATTERN = /(estorno|reembolso|devolver|devolucao|devolução|cancelar\s+pedido|quero\s+cancelar|quero\s+estorno|quero\s+reembolso)/i;
class RefundOrderByAdminService {
    ...
    async execute({ orderId, restaurantId, adminUserId, }) {
        ...
        if (!Number.isInteger(normalizedAdminUserId) ||
        ...
        if (!this.hasClientRefundRequest(issueThread)) {
            throw new Error("O cliente ainda não solicitou estorno no chat deste pedido.");
        }
        ...
        const updatedOrder = await prisma.$transaction(async (tx) => {
            ...
            return orderRepository.updateStatus(order.id, OrderStatus.CANCELADO, normalizedRestaurantId, tx);
        });
        ...
        if (resolvedPayload) {
            io.to(`restaurant:${normalizedRestaurantId}:admin`).emit("order:issue-resolved", resolvedPayload);
