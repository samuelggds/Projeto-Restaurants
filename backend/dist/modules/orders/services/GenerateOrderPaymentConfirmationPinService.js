...
class GenerateOrderPaymentConfirmationPinService {
    async execute(orderId, restaurantId) {
        ...
        if (!order.paymentMethod || !digitalMethods.has(order.paymentMethod)) {
            throw new Error("PIN de confirmação disponível apenas para PIX ou CARTAO.");
        }
        const pin = generateFourDigitPin();
        const expiresAt = new Date(Date.now() + 10 * 60 * 1000);
        const updatedOrder = await orderRepository.setPaymentConfirmationPin(orderId, restaurantId, pin, expiresAt);
