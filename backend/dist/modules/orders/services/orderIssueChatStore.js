...
export async function ensureOrderIssueThread(input) {
...
export async function getOrderIssueThread(orderId) {
    return loadThread(orderId);
}
export async function addOrderIssueMessage({ orderId, senderType, senderName, message, }) {
...
export async function resolveOrderIssueThread({ orderId, resolvedByName, }) {
    const thread = await prisma.orderIssueThread.findUnique({
        where: {
            orderId,
        },
...
export function toOrderIssueThreadPayload(thread) {
    if (!thread) {
        return null;
    }
    return {
