...
class ConfirmOrderPaymentService {
    async execute(orderId, restaurantId, role) {
        ...
        if (String(role || "").toUpperCase() === "MOTOQUEIRO") {
            throw new Error("Motoqueiro não pode confirmar pagamento direto. Use o PIN do dono/admin.");
        }
        ...
        if (!order.paymentMethod || !digitalMethods.has(order.paymentMethod)) {
            throw new Error("Confirmação manual de pagamento disponível apenas para PIX ou CARTAO.");
        }
        if (order.paid === true) {
            return order;
        }
