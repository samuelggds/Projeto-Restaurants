...
class RefundOrderPaymentService {
    ...
    resolvePagBankApiBaseUrl(environment) {
        void environment;
    ...
    parsePagBankErrorDetails(xml) {
        ...
        const code = this.extractXmlTagValue(errorScope, "code") ||
            this.extractXmlTagValue(normalizedXml, "code");
        const message = this.extractXmlTagValue(errorScope, "message") ||
    ...
    async getPagBankCredentials(restaurantId) {
        const settings = restaurantId
            ...
            : null;
        const email = String(settings?.pagbankEmail ||
            process.env.PAGBANK_EMAIL ||
            process.env.PAGSEGURO_EMAIL ||
            "").trim();
        const token = String(settings?.pagbankToken ||
            process.env.PAGBANK_TOKEN ||
            process.env.PAGSEGURO_TOKEN ||
            "").trim();
        if (!email || !token) {
            throw new Error("Credenciais PagBank nao configuradas. Nao foi possivel estornar automaticamente.");
        }
        return {
            email,
            token,
    ...
    async getMercadoPagoAccessTokenByRestaurant(restaurantId) {
        ...
        const settings = Number.isInteger(normalizedRestaurantId) && normalizedRestaurantId > 0
            ...
            : null;
        const token = String(settings?.mercadoPagoAccessToken || process.env.MP_ACCESS_TOKEN || "").trim();
        if (!token) {
            throw new Error("Credencial Mercado Pago nao configurada no restaurante. Nao foi possivel estornar automaticamente.");
        }
        return token;
    }
    async executeMercadoPagoRefund(paymentId, amount, restaurantId) {
        const paymentApi = (await getMercadoPagoPaymentApi(Number(restaurantId || 0) || undefined));
    ...
    async refundPix(order) {
        ...
        if (!paymentId) {
            throw new Error("Pedido PIX sem identificador de pagamento. Nao foi possivel estornar automaticamente.");
        }
        if (paymentId.startsWith("manual:")) {
            throw new Error("Pedido PIX manual exige estorno manual. Nao foi possivel estornar automaticamente.");
        }
    ...
    async refundStripeCard(order) {
        ...
        const settings = restaurantId
            ? await restaurantSettingsRepository.findByRestaurantId(restaurantId)
            : null;
        const secretKey = String(settings?.stripeSecretKey || process.env.STRIPE_SECRET_KEY || "").trim();
        if (!secretKey) {
            throw new Error("Chave Stripe nao configurada no restaurante. Nao foi possivel estornar automaticamente.");
        }
    ...
    async refundPagBankByTransaction(transactionCode, order) {
        ...
        if (!response.ok) {
            ...
            const detailsSuffix = parsedError.code
                ...
                : "";
            throw new Error(`PagBank refund [HTTP ${response.status}]: ${providerMessage}${detailsSuffix}`);
        }
        // Some PagBank responses return HTTP 200 with XML error payload.
        if (parsedError.code && parsedError.message) {
            throw new Error(`PagBank refund [HTTP ${response.status}]: ${parsedError.message} (code ${parsedError.code})`);
        }
    }
    async refundCard(order) {
        ...
        if (!checkoutSessionId) {
            throw new Error("Pedido CARTAO sem identificador de checkout. Nao foi possivel estornar automaticamente.");
        }
        ...
        if (checkoutSessionId.startsWith("mp_pref:")) {
            ...
            if (!preferenceId || !Number.isInteger(orderId) || orderId <= 0) {
                throw new Error("Pedido CARTAO Mercado Pago sem dados suficientes para localizar pagamento e estornar automaticamente.");
            }
            const externalReference = Number.isInteger(restaurantId) && restaurantId > 0
            ...
            if (!response.ok) {
                throw new Error("Falha ao consultar pagamento de cartao no Mercado Pago para estorno automatico.");
            }
            const resolvedPaymentId = String(payload?.results?.[0]?.id || "").trim();
            if (!resolvedPaymentId) {
                throw new Error("Nao foi possivel localizar o pagamento de cartao no Mercado Pago para estorno automatico.");
            }
        ...
        if (checkoutSessionId.startsWith("pagbank_chk:")) {
            throw new Error("Pedido PagBank ainda sem codigo de transacao confirmado para estorno automatico. Faca o estorno manual antes de cancelar.");
        }
