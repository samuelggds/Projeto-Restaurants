...
class FinalizeOrderCardPaymentService {
    async execute({ orderId, checkoutSessionId, restaurantId, allowMissingOrder = false, }) {
        ...
        const order = orderId
            ...
            : normalizedCheckoutSessionId
                ? await orderRepository.findByCardCheckoutSessionId(normalizedCheckoutSessionId, normalizedRestaurantId)
                : null;
        if (!order) {
