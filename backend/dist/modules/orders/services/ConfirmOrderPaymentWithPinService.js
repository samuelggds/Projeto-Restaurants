...
class ConfirmOrderPaymentWithPinService {
    async execute(orderId, restaurantId, role, pin) {
        ...
        if (!allowedRoles.includes(normalizedRole)) {
            throw new Error("A confirmação por PIN é permitida apenas para admin ou motoqueiro na entrega.");
        }
        ...
        if (!order.paymentMethod || !digitalMethods.has(order.paymentMethod)) {
            throw new Error("Confirmação por PIN disponível apenas para PIX ou CARTAO.");
        }
        if (order.paid === true) {
            return order;
        }
        ...
        if (!order.paymentConfirmationPin ||
