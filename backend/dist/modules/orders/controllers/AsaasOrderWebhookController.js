...
class AsaasOrderWebhookController {
    async handle(req, res) {
        try {
            ...
            const hasRequiredPaymentFields = Boolean(asaasPaymentId) &&
            ...
            const normalizedPaymentMethod = String(order.paymentMethod || "")
                .trim()
                .toUpperCase();
            const isSupportedAutomaticMethod = normalizedPaymentMethod === "PIX" ||
            ...
            if (walletId) {
                try {
                    ...
                    console.warn("[ASAAS_WEBHOOK_GATEWAY_ID_BACKFILL_ERROR]", settingsUpdateError instanceof Error
            ...
            console.error("[ASAAS_WEBHOOK_ERROR]", error instanceof Error ? error.message : String(error));
