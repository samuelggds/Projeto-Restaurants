...
function isBasicAuthDisabledError(error) {
    const message = error instanceof Error ? error.message : String(error || "");
    const normalized = message.toLowerCase();
    return (normalized.includes("535") &&
...
function getRequiredMfaRoles() {
    ...
    const envValue = rawEnvValue === undefined
...
function requiresMfa(role) {
    return getRequiredMfaRoles().has(String(role || "")
        .trim()
        .toUpperCase());
}
        ...
        if (transporter) {
            ...
            try {
                ...
                if (process.env.NODE_ENV !== "production") {
                    console.warn(`[login-2fa] Falha no SMTP em desenvolvimento. Codigo para ${user.email}: ${code}`);
                    return {
                        mfaRequired: true,
                        mfaToken: token,
                ...
                if (isBasicAuthDisabledError(error)) {
                    throw new Error("Falha no SMTP: o provedor bloqueou login por usuario/senha (basic auth). Configure SMTP_AUTH_TYPE=oauth2 com credenciais OAuth2 ou use app password.");
                }
                throw error;
            }
        }
            ...
            if (process.env.NODE_ENV === "production") {
                throw new Error("Falha no SMTP: configure SMTP_HOST, SMTP_PORT, SMTP_USER e credenciais validas para enviar o codigo 2FA por e-mail.");
            }
