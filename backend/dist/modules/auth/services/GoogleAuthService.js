...
class GoogleAuthService {
    async execute({ idToken }) {
        ...
        if (!googleClientId || googleClientIds.length === 0) {
            throw new Error("Login com Google indisponível. Configure GOOGLE_CLIENT_ID (ou GOOGLE_CLIENT_IDS) no backend.");
        }
        ...
        const mfaChallenge = await loginMfaService.beginIfRequired(user);
        if (mfaChallenge) {
            return mfaChallenge;
        }
        const tokenPayload = {
