...
function createOpenAiClient() {
    ...
    if (!apiKey) {
        throw new Error("OPENAI_API_KEY nao configurada. Adicione a chave no ambiente antes de usar o importador por imagem.");
    }
    ...
    return {
        ...
        categories: validated.data.categories.map((category) => ({
            ...
            items: category.items.map((item) => {
                ...
                if (price === null) {
                    throw new Error(`Preco invalido retornado pela OpenAI no item "${normalizeText(item.name)}".`);
                }
                return {
