...
function extractMenuFromCheerio($) {
    ...
    itemNodes.each((_, node) => {
        const root = $(node);
        const productName = normalizeText(root
            .find("h3, h4, h5, [class*='name'], [data-testid*='name']")
            .first()
            .text() || root.text());
        const description = normalizeText(root
            .find("p, [class*='description'], [data-testid*='description']")
            .first()
            .text());
        const priceText = normalizeText(root
        ...
        const categoryName = normalizeText(root
    ...
    if (!items.length && structuredData.length) {
        for (const entry of structuredData) {
            ...
            for (const graphItem of graph) {
                ...
                const description = normalizeText(graphItem.description);
                const priceValue = parsePrice(graphItem.price);
                if (name && priceValue) {
                    items.push({
    ...
    const dedupedItems = Array.from(new Map(items.map((item) => [
...
class ImportIfoodMenuScraperService {
    async execute(input) {
        ...
        if (!parsedMenu.items.length) {
            throw new Error("Não foi possível identificar categorias e produtos na página pública do iFood.");
        }
