...
export function validateCriticalEnv() {
    ...
    const loginLockoutAfterFailures = asNumber(String(process.env.LOGIN_LOCKOUT_AFTER_FAILURES || "5"), 5);
    if (loginLockoutAfterFailures < 3) {
        errors.push("LOGIN_LOCKOUT_AFTER_FAILURES deve ser >= 3 em producao.");
    }
    const loginLockoutBaseSeconds = asNumber(String(process.env.LOGIN_LOCKOUT_BASE_SECONDS || "60"), 60);
    ...
    const allowInsecureStripe = String(process.env.ALLOW_INSECURE_STRIPE_WEBHOOK || "false").trim() ===
        "true";
    if (allowInsecureStripe) {
        errors.push("ALLOW_INSECURE_STRIPE_WEBHOOK nao pode ser true em producao.");
    }
    const allowGlobalFallback = String(process.env.ALLOW_GLOBAL_PAYMENT_FALLBACK || "false").trim() ===
        "true";
