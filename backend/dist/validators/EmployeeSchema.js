...
import { z } from "zod";
const phoneRegex = /^(?:\+?55\s?)?(?:\(?([1-9][0-9])\)?\s?)?(?:((?:9\d|[2-9])\d{3})\s?-?\s?(\d{4}))$/;
export const EmployeeUserSchema = z
    ...
    role: z
        ...
        .optional()
        .refine((value) => !value ||
        value === UserRole.FUNCIONARIO ||
        value === UserRole.MOTOQUEIRO, {
    ...
    cpf: z
        ...
        /^\d{3}\.?\d{3}\.?\d{3}-?\d{2}$/.test(value
            .replace(/\D/g, "")
            .replace(/^(\d{3})(\d{3})(\d{3})(\d{2})$/, "$1.$2.$3-$4")), { message: "CPF inválido" }),
})
