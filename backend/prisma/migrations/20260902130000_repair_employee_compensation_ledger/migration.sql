...
-- The original 20260902120000 migration was replaced after some databases had
-- already applied its legacy employee-payment schema. Repair those databases
-- without changing databases that already contain the ledger schema.
ALTER TYPE "FuncionarioSubRole" ADD VALUE IF NOT EXISTS 'ATENDENTE';

DO $employee_compensation_enums$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_type t
    JOIN pg_namespace n ON n.oid = t.typnamespace
    WHERE n.nspname = current_schema() AND t.typname = 'EmployeeCompensationBaseModel'
  ) THEN
    CREATE TYPE "EmployeeCompensationBaseModel" AS ENUM ('NONE', 'FIXED_MONTHLY', 'HOURLY');
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_type t
    JOIN pg_namespace n ON n.oid = t.typnamespace
    WHERE n.nspname = current_schema() AND t.typname = 'EmployeeCompensationVariableModel'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_type t
    JOIN pg_namespace n ON n.oid = t.typnamespace
    WHERE n.nspname = current_schema() AND t.typname = 'EmployeeCompensationProrationMode'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_type t
    JOIN pg_namespace n ON n.oid = t.typnamespace
    WHERE n.nspname = current_schema() AND t.typname = 'EmployeeWorkEntryStatus'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_type t
    JOIN pg_namespace n ON n.oid = t.typnamespace
    WHERE n.nspname = current_schema() AND t.typname = 'EmployeeEarningType'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_type t
    JOIN pg_namespace n ON n.oid = t.typnamespace
    WHERE n.nspname = current_schema() AND t.typname = 'EmployeeEarningDirection'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_type t
    JOIN pg_namespace n ON n.oid = t.typnamespace
    WHERE n.nspname = current_schema() AND t.typname = 'EmployeeEarningSourceType'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_type t
    JOIN pg_namespace n ON n.oid = t.typnamespace
    WHERE n.nspname = current_schema() AND t.typname = 'EmployeeSettlementStatus'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_type t
    JOIN pg_namespace n ON n.oid = t.typnamespace
    WHERE n.nspname = current_schema() AND t.typname = 'EmployeeSettlementPaymentMethod'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_type t
    JOIN pg_namespace n ON n.oid = t.typnamespace
    WHERE n.nspname = current_schema() AND t.typname = 'EmployeeSettlementPaymentStatus'
  ) THEN
    CREATE TYPE "EmployeeSettlementPaymentStatus" AS ENUM ('ACTIVE', 'REVERSED');
  END IF;
END
$employee_compensation_enums$;

CREATE UNIQUE INDEX IF NOT EXISTS "User_id_restaurantId_key"
...
CREATE TABLE IF NOT EXISTS "TableWaiterAssignment" (
  "id" SERIAL NOT NULL,
  "publicId" TEXT NOT NULL DEFAULT gen_random_uuid()::text,
  "restaurantId" INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS "EmployeeWorkEntry" (
  "id" SERIAL NOT NULL,
  "publicId" TEXT NOT NULL DEFAULT gen_random_uuid()::text,
  "restaurantId" INTEGER NOT NULL,
  "employeeId" INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS "EmployeeEarning" (
  "id" SERIAL NOT NULL,
  "publicId" TEXT NOT NULL DEFAULT gen_random_uuid()::text,
  "restaurantId" INTEGER NOT NULL,
  "employeeId" INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS "EmployeeSettlementItem" (
  "id" SERIAL NOT NULL,
  "publicId" TEXT NOT NULL DEFAULT gen_random_uuid()::text,
  "restaurantId" INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS "EmployeeSettlementPayment" (
  "id" SERIAL NOT NULL,
  "publicId" TEXT NOT NULL DEFAULT gen_random_uuid()::text,
  "restaurantId" INTEGER NOT NULL,
  "settlementId" INTEGER NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeCompensationPolicy_publicId_key" ON "EmployeeCompensationPolicy"("publicId");
CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeCompensationPolicy_id_restaurantId_key" ON "EmployeeCompensationPolicy"("id", "restaurantId");
CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeCompensationPolicy_restaurantId_employeeId_version_key" ON "EmployeeCompensationPolicy"("restaurantId", "employeeId", "version");
CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeCompensationPolicy_one_active_per_employee_idx" ON "EmployeeCompensationPolicy"("restaurantId", "employeeId") WHERE "active" = true;
CREATE INDEX IF NOT EXISTS "EmployeeCompensationPolicy_restaurantId_employeeId_active_effectiveFrom_idx" ON "EmployeeCompensationPolicy"("restaurantId", "employeeId", "active", "effectiveFrom");
CREATE INDEX IF NOT EXISTS "EmployeeCompensationPolicy_restaurantId_effectiveFrom_effectiveUntil_idx" ON "EmployeeCompensationPolicy"("restaurantId", "effectiveFrom", "effectiveUntil");

CREATE UNIQUE INDEX IF NOT EXISTS "TableWaiterAssignment_publicId_key" ON "TableWaiterAssignment"("publicId");
CREATE UNIQUE INDEX IF NOT EXISTS "TableWaiterAssignment_id_restaurantId_key" ON "TableWaiterAssignment"("id", "restaurantId");
CREATE UNIQUE INDEX IF NOT EXISTS "TableWaiterAssignment_one_active_per_session_idx" ON "TableWaiterAssignment"("restaurantId", "tableSessionId") WHERE "unassignedAt" IS NULL;
CREATE INDEX IF NOT EXISTS "TableWaiterAssignment_restaurantId_tableSessionId_assignedAt_idx" ON "TableWaiterAssignment"("restaurantId", "tableSessionId", "assignedAt");
CREATE INDEX IF NOT EXISTS "TableWaiterAssignment_restaurantId_waiterId_unassignedAt_idx" ON "TableWaiterAssignment"("restaurantId", "waiterId", "unassignedAt");

CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeWorkEntry_publicId_key" ON "EmployeeWorkEntry"("publicId");
CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeWorkEntry_id_restaurantId_key" ON "EmployeeWorkEntry"("id", "restaurantId");
CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeWorkEntry_one_current_per_employee_day_idx" ON "EmployeeWorkEntry"("restaurantId", "employeeId", "workDate") WHERE "status" <> 'CANCELED';
CREATE INDEX IF NOT EXISTS "EmployeeWorkEntry_restaurantId_employeeId_workDate_idx" ON "EmployeeWorkEntry"("restaurantId", "employeeId", "workDate");
CREATE INDEX IF NOT EXISTS "EmployeeWorkEntry_restaurantId_status_workDate_idx" ON "EmployeeWorkEntry"("restaurantId", "status", "workDate");

CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeEarning_publicId_key" ON "EmployeeEarning"("publicId");
CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeEarning_id_restaurantId_key" ON "EmployeeEarning"("id", "restaurantId");
CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeEarning_restaurantId_employeeId_sourceType_sourceId_type_key" ON "EmployeeEarning"("restaurantId", "employeeId", "sourceType", "sourceId", "type");
CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeEarning_restaurantId_idempotencyKeyHash_key" ON "EmployeeEarning"("restaurantId", "idempotencyKeyHash");
CREATE INDEX IF NOT EXISTS "EmployeeEarning_restaurantId_employeeId_occurredAt_idx" ON "EmployeeEarning"("restaurantId", "employeeId", "occurredAt");
CREATE INDEX IF NOT EXISTS "EmployeeEarning_restaurantId_tableSessionId_type_idx" ON "EmployeeEarning"("restaurantId", "tableSessionId", "type");
CREATE INDEX IF NOT EXISTS "EmployeeEarning_restaurantId_settledAt_occurredAt_idx" ON "EmployeeEarning"("restaurantId", "settledAt", "occurredAt");

CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeSettlement_publicId_key" ON "EmployeeSettlement"("publicId");
CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeSettlement_id_restaurantId_key" ON "EmployeeSettlement"("id", "restaurantId");
CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeSettlement_id_restaurantId_employeeId_key" ON "EmployeeSettlement"("id", "restaurantId", "employeeId");
CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeSettlement_restaurantId_employeeId_periodYear_periodMonth_key" ON "EmployeeSettlement"("restaurantId", "employeeId", "periodYear", "periodMonth");
CREATE INDEX IF NOT EXISTS "EmployeeSettlement_restaurantId_status_periodStart_idx" ON "EmployeeSettlement"("restaurantId", "status", "periodStart");

CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeSettlementItem_publicId_key" ON "EmployeeSettlementItem"("publicId");
CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeSettlementItem_id_restaurantId_key" ON "EmployeeSettlementItem"("id", "restaurantId");
CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeSettlementItem_settlementId_earningId_key" ON "EmployeeSettlementItem"("settlementId", "earningId");
CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeSettlementItem_one_active_per_earning_idx" ON "EmployeeSettlementItem"("restaurantId", "earningId") WHERE "active" = true;
CREATE INDEX IF NOT EXISTS "EmployeeSettlementItem_restaurantId_earningId_active_idx" ON "EmployeeSettlementItem"("restaurantId", "earningId", "active");

CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeSettlementPayment_publicId_key" ON "EmployeeSettlementPayment"("publicId");
CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeSettlementPayment_id_restaurantId_key" ON "EmployeeSettlementPayment"("id", "restaurantId");
CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeSettlementPayment_restaurantId_idempotencyKeyHash_key" ON "EmployeeSettlementPayment"("restaurantId", "idempotencyKeyHash");
CREATE INDEX IF NOT EXISTS "EmployeeSettlementPayment_restaurantId_settlementId_status_registeredAt_idx" ON "EmployeeSettlementPayment"("restaurantId", "settlementId", "status", "registeredAt");

DO $employee_compensation_constraints$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'EmployeeCompensationPolicy_restaurantId_fkey') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'EmployeeCompensationPolicy_employeeId_restaurantId_fkey') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'TableWaiterAssignment_restaurantId_fkey') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'TableWaiterAssignment_tableSessionId_restaurantId_fkey') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'TableWaiterAssignment_waiterId_restaurantId_fkey') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'EmployeeWorkEntry_restaurantId_fkey') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'EmployeeWorkEntry_employeeId_restaurantId_fkey') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'EmployeeEarning_restaurantId_fkey') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'EmployeeEarning_employeeId_restaurantId_fkey') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'EmployeeEarning_policyId_restaurantId_fkey') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'EmployeeEarning_tableSessionId_restaurantId_fkey') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'EmployeeEarning_paymentIntentId_restaurantId_tableSessionId_fkey') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'EmployeeEarning_reversesEarningId_restaurantId_fkey') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'EmployeeSettlement_restaurantId_fkey') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'EmployeeSettlement_employeeId_restaurantId_fkey') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'EmployeeSettlementItem_restaurantId_fkey') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'EmployeeSettlementItem_settlementId_restaurantId_fkey') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'EmployeeSettlementItem_earningId_restaurantId_fkey') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'EmployeeSettlementPayment_restaurantId_fkey') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'EmployeeSettlementPayment_settlementId_restaurantId_employeeId_fkey') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'EmployeeSettlementPayment_employeeId_restaurantId_fkey') THEN
  ...
  END IF;
END
$employee_compensation_constraints$;

DO $employee_compensation_rls$
DECLARE
  table_name text;
BEGIN
