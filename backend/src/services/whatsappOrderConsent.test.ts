...
test('registra consentimento transacional sem persistir telefone ou segredo', async () => {
  let created = null;
  ...
  assert.equal(JSON.stringify(created).includes('phone'), false);
...
test('não duplica a evidência quando o mesmo pedido é repetido por idempotência', async () => {
  ...
  assert.equal(
    await recordWhatsappOrderNotificationOptIn({ restaurantId: 12, orderId: 44, userId: 7 }),
    true,
  );
  assert.equal(createCalls, 0);
});

test('consulta consentimento sempre pelo restaurante e pedido exatos', async () => {
