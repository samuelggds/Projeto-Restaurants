...
function getSecret() {
  ...
  if (process.env.NODE_ENV === 'production' && secret.length < 32) {
    throw new Error('PAYMENT_PIN_SECRET deve ter pelo menos 32 caracteres em produção.');
  }
...
export function hashPaymentConfirmationPin(pin: string) {
  const digest = crypto.createHmac('sha256', getSecret()).update(String(pin)).digest('hex');
