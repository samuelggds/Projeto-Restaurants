...
class GenerateOrderPaymentConfirmationPinController {
  async handle(req: Request, res: Response) {
    try {
      ...
      const result = await generateOrderPaymentConfirmationPinService.execute(id, restaurantId);

      return res.status(200).json(result);
    } catch (error: unknown) {
      return res.status(400).json({
        error:
          error instanceof Error ? error.message : 'Erro ao gerar PIN de confirmacao de pagamento',
      });
    }
  }
}
