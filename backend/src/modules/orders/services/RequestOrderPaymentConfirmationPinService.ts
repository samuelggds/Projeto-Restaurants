...
class RequestOrderPaymentConfirmationPinService {
  async execute(orderId: number | string, restaurantId: number, role: UserRole | string, actorUserId?: number | null) {
    ...
    if (!allowedRoles.includes(normalizedRole as (typeof allowedRoles)[number])) {
      throw new Error('Somente admin ou motoqueiro podem solicitar PIN de confirmação de pagamento.');
    }
    const order = await orderRepository.findById(orderId, restaurantId);
    if (!order) throw new Error('Pedido não encontrado!');
    if (String(order.type || '').toUpperCase() !== 'DELIVERY') throw new Error('Solicitação de PIN disponível apenas para pedidos DELIVERY.');
    if (String(order.status || '').toUpperCase() !== 'SAIU_PARA_ENTREGA') throw new Error('Solicitação de PIN disponível apenas durante a entrega.');
    if (normalizedRole === UserRole.MOTOQUEIRO) {
      ...
      if (Number(order.assignedCourierId || 0) !== courierId) throw new Error('Esta entrega não está atribuída a você.');
    }
    ...
    if (order.payOnDelivery !== true || order.paymentMethod !== PaymentMethod.DINHEIRO) {
      throw new Error('Solicitação de PIN disponível apenas para recebimento em dinheiro na entrega.');
    }
    ...
    notifyRestaurantPaymentPinRequested({
      ...
      console.error('[RESTAURANT_PIN_NOTIFICATION_UNHANDLED]', error instanceof Error ? error.message : String(error));
    });
    ...
    return { orderId: order.id, requestedAt, message: 'Solicitação de PIN enviada para o dono/admin.' };
  }
}
