...
const req = (body: unknown, userId = 7) =>
  ({
    headers: { 'idempotency-key': 'attempt-key-123456789' },
    user: { id: userId },
    body,
...
test('fingerprint é estável e o escopo usa a identidade autenticada', () => {
  ...
  assert.notEqual(
    first.fingerprint,
    orderCreationContext(req({ restaurantId: 2, items: [1, 2] }))?.fingerprint,
  );
  assert.notEqual(
    orderCreationContext(req({}), 'pix')?.fingerprint,
    orderCreationContext(req({}), 'card')?.fingerprint,
  );
  assert.throws(
    () =>
      orderCreationContext({
...
test('replay consulta restaurante/ator/chave, devolve o pedido e recusa conteúdo conflitante', async () => {
  ...
  try {
    ...
    await assert.rejects(replayCreatedOrder(db, 3, context), {
      statusCode: 409,
      code: 'IDEMPOTENCY_CONFLICT',
    });
  } finally {
    orderRepository.findById = original;
  }
});
