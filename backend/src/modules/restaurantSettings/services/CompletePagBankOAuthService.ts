...
class CompletePagBankOAuthService {
  async execute({ code, state }: { code?: string; state?: string }) {
    ...
    if (!response.ok || !accessToken) {
      throw new Error(
        String(body.error_description || body.error || "PagBank recusou a conexão."),
      );
    }
    const refreshToken = String(body.refresh_token || "").trim() || null;
    const expiresIn = Number(body.expires_in || 0);
    const expiresAt = expiresIn > 0
      ? new Date(Date.now() + Math.max(expiresIn - 300, 60) * 1000)
      : null;
    ...
    else await restaurantSettingsRepository.create({
      restaurantId,
      deliveryFee: 0,
      minimumOrder: 0,
      ...data,
