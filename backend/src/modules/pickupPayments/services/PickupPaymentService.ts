...
class PickupPaymentService {
  private assertOperator(role: string, subRole?: string | null) {
    ...
    if (!allowed)
      throw new Error('Somente administrador ou atendente pode receber retirada no balcão.');
  }
  ...
  private async getPickup(orderId: number, restaurantId: number) {
    ...
    if (!order || order.type !== OrderType.RETIRADA)
      throw new Error('Pedido de retirada não encontrado.');
  ...
  async start(input: {
    ...
    if (
      input.method === 'CARTAO' &&
      String(settings?.cardGateway).toUpperCase() !== 'MERCADO_PAGO'
    ) {
    ...
    const reservation = await withLockedPickup(
      order.id,
      order.restaurantId,
      async (tx, current) => {
        if (current.paid) return { order: current, payment: null, created: false };
        if (current.paymentMethod)
          throw new Error('Este pedido já possui pagamento online definido.');
        const existing = await findPickupPayment(tx, order.id, order.restaurantId);
        if (existing) {
          if (existing.method !== input.method)
            throw new Error('Concilie a cobrança existente antes de trocar a forma de pagamento.');
          if (existing.provider !== provider)
            throw new Error('O provedor mudou. Concilie a cobrança anterior antes de continuar.');
          ...
          if (cents(existing.amount) !== cents(current.total) || existing.currency !== 'BRL') {
            throw new Error(
              'O valor da tentativa diverge do pedido. Concilie a cobrança existente.',
            );
          }
        ...
        if (
          input.method === 'CARTAO' &&
          (!requestedTerminal?.active ||
            String(requestedTerminal.operatingMode).toUpperCase() !== 'PDV')
        ) {
        ...
        const payment = await reservePickupPayment(tx, {
          ...
          terminalId: requestedTerminal?.id,
        });
    ...
    if (
      (payment.method === 'PIX' && payment.providerPaymentId && payment.pixCopyPaste) ||
      (payment.method === 'CARTAO' && payment.providerOrderId) ||
      payment.status === 'PAID'
    ) {
    ...
    if (
      ...
      throw new Error(
        'A tentativa anterior precisa de conciliação com o provedor. Nenhuma nova cobrança foi criada.',
      );
    }
    ...
    try {
      ...
      if (!terminal || terminal.active !== true)
        throw new Error('Selecione uma maquininha ativa do restaurante.');
      ...
      await savePickupProvider({ payment, lastProviderStatus: 'creation_unknown:v1' }).catch(
        () => undefined,
      );
      ...
      throw new Error(
        'Não foi possível concluir a cobrança. A tentativa foi preservada; consulte ou retome este pagamento antes de cobrar novamente.',
      );
    }
  }
  ...
  private async confirm(
    ...
    const result = await withLockedPickup(orderId, restaurantId, async (tx, order) => {
      ...
      if (
        ...
        payment &&
        !['FAILED', 'CANCELLED', 'CANCELED', 'EXPIRED'].includes(payment.status)
      ) {
        throw new Error('Concilie a cobrança pendente antes de receber em dinheiro.');
      }
      if (
        method !== PaymentMethod.DINHEIRO &&
        (!payment || payment.publicId !== approvedPayment?.publicId || payment.method !== method)
      ) {
        throw new Error('A confirmação não corresponde à tentativa de pagamento atual.');
      }
      if (order.paid) {
        if (order.paymentMethod !== method) {
          if (order.paymentMethod !== null || !approvedPayment) {
            throw new Error('O pedido já foi pago por outra forma de pagamento.');
          }
          // Recover historical pickup webhooks that set paid without a method.
          await tx.order.updateMany({
            where: { id: orderId, restaurantId, paid: true, paymentMethod: null },
      ...
      if (method === PaymentMethod.PIX && providerPaymentId) {
        await orderRepository.claimPixPaymentId(orderId, restaurantId, providerPaymentId, tx);
      }
      ...
      if (changed.count !== 1)
        throw new Error('O pedido mudou durante a confirmação. Consulte o estado atual.');
  ...
  async reconcile(input: {
    ...
    if (order.paid === true && (!payment || payment.status === 'PAID'))
      return { paid: true, order, payment: publicPayment(payment) };
  ...
  private async reconcilePersistedPayment(
    ...
    if (String(payment.method).toUpperCase() === 'PIX') {
      ...
      const updatedOrder = await this.confirm(
        order.id,
        order.restaurantId,
        PaymentMethod.PIX,
        payment,
      ...
      return {
        ...
        payment: publicPayment(
          await paymentTerminalRepository.findDeliveryPayment(order.id, order.restaurantId),
        ),
      };
    }
    ...
    if (
      String(providerOrder?.external_reference || '') !== `pickup-${order.restaurantId}-${order.id}`
    ) {
    ...
    if (
      ...
      transactionPaid !== expectedAmount
    ) {
  ...
  /** Internal bridge: caller has validated gateway reference, amount and currency. */
  async confirmApprovedPix(orderId: number, restaurantId: number, paymentId: string) {
    ...
    if (
      ...
      throw new Error('O PIX aprovado não corresponde à cobrança da retirada.');
    }
    return this.confirm(orderId, restaurantId, PaymentMethod.PIX, payment, paymentId);
  }

  /** Called only after webhook authentication and lookup by provider order ID. */
  async reconcilePointWebhook(payment: PickupPayment) {
    const current = await orderRepository.findById(payment.orderId, payment.restaurantId);
    if (!current || current.type !== OrderType.RETIRADA) return false;
    if (
      payment.method !== 'CARTAO' ||
      payment.provider !== 'MERCADO_PAGO' ||
      !payment.providerOrderId
    ) {
      throw new Error('A notificação Point não corresponde à cobrança da retirada.');
    }
    const order = await this.getPickup(payment.orderId, payment.restaurantId);
    await this.reconcilePersistedPayment(order, payment);
    return true;
  }
}
