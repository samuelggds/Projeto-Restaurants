  ...
  async handle(req: Request, res: Response) {
    try {
      ...
      const rawRestaurantId =
        Array.isArray(req.query.restaurantId) && req.query.restaurantId.length > 0
      ...
      const queryRestaurantId =
        typeof rawRestaurantId === 'string' || typeof rawRestaurantId === 'number'
          ...
          : null;
      const rawBeforeId =
        Array.isArray(req.query.beforeId) && req.query.beforeId.length > 0
          ? req.query.beforeId[0]
          : req.query.beforeId || null;
      const queryBeforeId =
        typeof rawBeforeId === 'string' || typeof rawBeforeId === 'number' ? rawBeforeId : null;
      const rawLimit =
        Array.isArray(req.query.limit) && req.query.limit.length > 0
          ? req.query.limit[0]
      ...
      return res.status(400).json({
        error:
          error instanceof Error ? error.message : 'Erro ao carregar histórico do chat de suporte.',
      });
    }
  }
}
